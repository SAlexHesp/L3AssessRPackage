...
#' @param ubnd upper bounds of length classes
#' @param midpt mid points of length classes
#' @param SelectivityAtLen selectivity at length
                                             ...
                                             lbnd, ubnd, midpt, SelectivityAtLen, PropReleased, ObsDiscCatchFreqAtLen, DiscMort, CVSizeAtAge, MaxAge, NatMort, TimeStep)
{
  ...
  CV_LenAtMaxAge_Results = GetMaxLenAtAgeCV_AgeAndLengthBasedCatchCurve(TimeStep, res, MaxAge, SelectivityType)
  ...
  ModelDiag = list(midpt=res$midpt,
                   ...
                   SelAtLength = res$SelectivityAtLen,
...
GetSelectParams_AgeAndLengthBasedCatchCurvesCalcs <- function(Res, params, CatchCurveType, SelectivityType) {
  ...
  if (SelectivityType == 2) { # logistic selectivity
    ...
    if (CatchCurveType==2) { # length-based catch curve
      if (length(params)==10) { # estimating sex-specific length-based logistic selectivity
        L50 = exp(params[2:3])
        L95 = exp(params[4:5])
      }
    }
    ...
    if (CatchCurveType==1) { # length-based catch curve
      if (length(params)==5) { # estimating logistic selectivity and logistic retention (using discard and retained catch data)
  ...
  if (SelectivityType == 3) { # separate selectivity by fleet, with common growth curve
    L50 = exp(params[2:3])
    L95 = exp(params[4:5])
  }
...
GetGrowthParams_AgeAndLengthBasedCatchCurvesCalcs <- function(params, GrowthCurveType, SelectivityType) {
  ...
  if (GrowthCurveType == 1) { # von Bertalanffy
    if (SelectivityType == 1) { # selectivity type = 1 - direct input
    ...
    if (SelectivityType==2) { # selectivity type = 2 - logistic selectivity
      ...
      if (length(params)==10) { # 2 sexes, separate logistic selectivity parameters
        Linf = exp(params[6:7])
        vbK = exp(params[8:9])
        GrowthParams = data.frame(Linf=Linf,vbK=vbK)
    ...
    } # selectivity type = 2 - logistic selectivity

    if (SelectivityType==3) { # selectivity type = 3 - fleet-specific selectivity, common growth
      Linf = exp(params[6])
      vbK = exp(params[7])
      GrowthParams = c(Linf,vbK)
      CVSizeAtAge = c(exp(params[8]),exp(params[8]))
...
#' @param nLenCl number of length classes
#' @param midpt mid points of length classes
#' @param GrowthCurveType 1=von Bertalanffy, 2=Schnute, 3=any model with annual size increment input
#' @param GrowthModelType 1=von Bertalanffy - 1 sex, 2=von Bertalanffy - 2 sexes,
#' 3=Schnute - 1 sex, 4=Schnute - 2 sexes, 5=size increment input - 1 sex, 6=size increment input - 1 sexes
#' @param GrowthParams growth parameters or list with tag-model growth outputs
#' @param RefnceAges Schnute reference ages
#' @param SelectivityType 1=selectivity inputted as vector, 2=asymptotic logistic selectivity curve
#'
...
GetGrowthInputsForLengthTransitionMatrices <- function(MaxAge, TimeStep, nLenCl, midpt, GrowthCurveType, GrowthModelType, GrowthParams, RefnceAges, SelectivityType) {
  ...
  if (GrowthModelType == 1) { # von Bertalanffy, single sex
    ...
    if (SelectivityType != 3) {
      ...
      for (i in 1:2) {
        MeanSizeAtAge[i,] = Linf * (1 - exp (-vbK * Ages))
        MeanEndingLength[i,] = midpt + (Linf - midpt) * (1 - exp(-vbK*TimeStep))
  ...
  if (GrowthModelType == 5) { # annual size increment input, single sex
    MeanSizeAtAge = GrowthParams$MeanSizeAtAge
    MeanEndingLength = GrowthParams$MeanEndingLength
    TimestepGrowthSizeInc = GrowthParams$TimestepGrowthSizeInc # amount of annual growth with respect to initial length
  }

  if (GrowthModelType == 6) { # annual size increment input, single sex
    MeanSizeAtAge <- data.frame(matrix(nrow = 2, ncol = length(Ages)))
    colnames(MeanSizeAtAge) <- Ages
    MeanEndingLength <- data.frame(matrix(nrow = 2, ncol = nLenCl))
    colnames(MeanEndingLength) <- midpt
    TimestepGrowthSizeInc = MeanEndingLength
    for (i in 1:2) {
    MeanSizeAtAge[i,] = GrowthParams$MeanSizeAtAge[i,]
    MeanEndingLength[i,] = GrowthParams$MeanEndingLength[i,]
    TimestepGrowthSizeInc[i,] = GrowthParams$TimestepGrowthSizeInc[i,]
    }
  }
...
GetGrowthModelType <- function (GrowthCurveType, GrowthParams)
{
  ...
  if (GrowthCurveType==1) { # von Bertalanffy
    if (is.vector(GrowthParams)) {
      GrowthModelType = 1 # one sex
    } else {
      GrowthModelType = 2 # two sexes
    }
  }
  if (GrowthCurveType==2) { # Schnute
    if (is.vector(GrowthParams)) {
  ...
  if (GrowthCurveType==3) { # Any model with mean size increment input
    if (is.vector(GrowthParams)) {
      GrowthModelType = 5
    } else {
      GrowthModelType = 6
...
CalcSelectivityPenalties <- function (Res, GrowthCurveType, GrowthModelType, GrowthParams, RefnceAges, SelectivityType, SelParams) {
  ...
  if (GrowthCurveType==3) { # annual length increment input
    if (GrowthModelType == 5) { # single sex
      Linf = max(GrowthParams$MeanSizeAtAge)
    } else { # 2 sexes, von Bertalanffy
      Linf = max(c(GrowthParams$MeanSizeAtAge[1,],GrowthParams$MeanSizeAtAge[2,]))
    }
  }
  ...
  if (SelectivityType == 2) { # logistic selectivity
    ...
    if (length(SelParams)==4) { # separate selectivity for sexes

      if (sum(c(SelParams[1],SelParams[3])) > max(Linf)) {
        L95_Pen = (sum(c(SelParams[1],SelParams[3])) - max(Linf))^2
        SelParams[3] = max(Linf) - SelParams[1]
      }
      if (sum(c(SelParams[2],SelParams[4])) > max(Linf)) {
        L95_Pen = L95_Pen + (sum(c(SelParams[2],SelParams[4])) - max(Linf))^2
        SelParams[4] = max(Linf) - SelParams[2]
      }
    }

    if (length(SelParams)==2) { # combined selectivity for sexes
      if (sum(SelParams) > max(Linf)) {
        L95_Pen = 10 * (sum(SelParams) - max(Linf))^2
        SelParams[2] = max(Linf) - SelParams[1]
      }

      # calculate L50 penalty
      if (SelParams[1] > (sum(SelParams) - 2)) {
        L50_Pen = 10 * ((sum(SelParams) - 2) - SelParams[1])^2
                                               ...
                                               MLL, SelectivityType, lbnd, ubnd, midpt, SelectivityAtLen, DiscMort, MaxAge, NatMort, TimeStep)
{
  ...
  SelRes = GetSelectivityAndRetention(midpt, SelectivityType, SelParams, SelectivityAtLen, RetenParams, MLL)
  SelectivityAtLen = SelRes$SelectivityAtLen; RetAtLength = SelRes$RetAtLength
  ...
  if (SelectivityType !=3) {
    if (GrowthModelType ==  1 | GrowthModelType == 3 | GrowthModelType == 5) { # single sex
  ...
  if (GrowthModelType ==  2 | GrowthModelType == 4 | GrowthModelType == 6 | SelectivityType == 3) { # separate sex
    ...
    InitRecNumber = 0.5
    if (is.vector(SelectivityAtLen)) { # single selectivity curve for combined sexes
      SelAtLength = SelectivityAtLen
    } else { # separate sex
    ...
    # selectivity of landings at length
    SelLandings_Fem = CatchCurveResults_Fem$SelLandAtLength; SelLandings_Mal = CatchCurveResults_Mal$SelLandAtLength
    SelLandAtLength = t(data.frame(SelLandings_Fem=SelLandings_Fem, SelLandings_Mal=SelLandings_Mal))
  ...
  Results = list(SelectivityAtLen = SelectivityAtLen,
...
#' ObsRetCatchFreqAtLen <- data.frame(matrix(nrow = 2, ncol = length(midpt))) # 2 sex
#' colnames(ObsRetCatchFreqAtLen) <- midpt
#' ObsRetCatchFreqAtLen[1,] = SimRes$ObsRetCatchFreqAtLen_Fem
#' ObsRetCatchFreqAtLen[2,] = SimRes$ObsRetCatchFreqAtLen_Mal
                                                   ...
                                                   lbnd, ubnd, midpt, SelectivityAtLen, DiscMort, MaxAge, NatMort, TimeStep)
{
  ...
  ModelDiag = list(params = nlmb$par,
                   vcov.Params = vcov.Params,
                   cor.Params = cor.Params,
                   SelectivityAtLen = res$SelectivityAtLen,
                   RetAtLength = res$RetAtLength,
                   SelLandAtLength = res$SelLandAtLength,
                   MeanSizeAtAge = res$MeanSizeAtAge,
                   midpt = res$midpt,
                   ...
                   RetCatchAtIntAge_Fem = res$RetCatchAtIntAge_Fem,
                   RetCatchAtIntAge_Mal = res$RetCatchAtIntAge_Mal,
                   ...
                   L95_Pen = res$L95_Pen,
                   GrowthModelType = res$GrowthModelType,
                   TotCatchAtIntAgeLen = res$TotCatchAtIntAgeLen,
                   TotCatchAtIntAgeLen_Fem = res$TotCatchAtIntAgeLen_Fem,
                   TotCatchAtIntAgeLen_Mal = res$TotCatchAtIntAgeLen_Mal,
                   RetCatchAtIntAgeLen = res$RetCatchAtIntAgeLen,
                   RetCatchAtIntAgeLen_Fem = res$RetCatchAtIntAgeLen_Fem,
                   RetCatchAtIntAgeLen_Mal = res$RetCatchAtIntAgeLen_Mal,
                   DiscCatchAtIntAgeLen = res$DiscCatchAtIntAgeLen,
                   DiscCatchAtIntAgeLen_Fem = res$DiscCatchAtIntAgeLen_Fem,
                   DiscCatchAtIntAgeLen_Mal = res$DiscCatchAtIntAgeLen_Mal,
                   TotCatchAtLen = res$TotCatchAtLen,
                   TotCatchAtLen_Fem = res$TotCatchAtLen_Fem,
                   TotCatchAtLen_Mal = res$TotCatchAtLen_Mal,
                   RetCatchAtLen = res$RetCatchAtLen,
                   RetCatchAtLen_Fem = res$RetCatchAtLen_Fem,
                   RetCatchAtLen_Mal = res$RetCatchAtLen_Mal,
                   DiscCatchAtLen = res$DiscCatchAtLen,
                   DiscCatchAtLen_Fem = res$DiscCatchAtLen_Fem,
                   RetCatchAtLen_Mal = res$RetCatchAtLen_Mal,
                   FAtLen = res$FAtLen,
                   FAtLen_Fem = res$FAtLen_Fem,
                   FAtLen_Mal = res$FAtLen_Mal,
                   ...
                   ZAtLen_Fem = res$ZAtLen_Fem,
                   ZAtLen_Mal = res$ZAtLen_Mal,
                   FAtLenCapt = res$FAtLenCapt,
                   FAtLenCapt_Fem = res$FAtLenCapt_Fem,
                   FAtLenCapt_Mal = res$FAtLenCapt_Mal,
                   FAtLenReten = res$FAtLenReten,
                   FAtLenReten_Fem = res$FAtLenReten_Fem,
                   FAtLenReten_Mal = res$FAtLenReten_Mal,
                   FAtLenDisc = res$FAtLenDisc,
...
#'
#' @return ParamEst
GetMaxLenAtAgeCV_AgeAndLengthBasedCatchCurve <- function (TimeStep, res, MaxAge, SelectivityType)
{

  GrowthModelType = res$GrowthModelType # 1=vB, single sex, 2=vB
  # single sex, 3=Schnute 1 sex, 4=Schnute 2 sexes, 5 = ann inc 1 sex, 6 = ann inc 2 sexes
...
GetParamRes_AgeAndLengthBasedCatchCurve <- function (GrowthCurveType, SelectivityType, params, nlmb, ses)
{
  ...
  if (GrowthCurveType == 1) { # von Bertalanffy
    ...
    if (SelectivityType == 2 & length(params)==10) { # sep sex selectivity vector input, separate sex growth # logistic selectivity, separate sex input
      EstL50_1 = exp(c(nlmb$par[2], nlmb$par[2] + c(-1.96, 1.96) * ses[2]))
      EstL50_2 = exp(c(nlmb$par[3], nlmb$par[3] + c(-1.96, 1.96) * ses[3]))
      EstDelta_1 = exp(c(nlmb$par[4], nlmb$par[4] + c(-1.96, 1.96) * ses[4]))
      EstDelta_2 = exp(c(nlmb$par[5], nlmb$par[5] + c(-1.96, 1.96) * ses[5]))
      EstLinf_F = exp(c(nlmb$par[6], nlmb$par[6] + c(-1.96, 1.96) * ses[6]))
      EstLinf_M = exp(c(nlmb$par[7], nlmb$par[7] + c(-1.96, 1.96) * ses[7]))
      Estk_F = exp(c(nlmb$par[8], nlmb$par[8] + c(-1.96, 1.96) * ses[8]))
      Estk_M = exp(c(nlmb$par[9], nlmb$par[9] + c(-1.96, 1.96) * ses[9]))
      EstCV = exp(c(nlmb$par[10], nlmb$par[10] + c(-1.96, 1.96) * ses[10]))
      ParamEst = t(data.frame(FMort = round(EstFMort, 3), L50_1 = round(EstL50_1, 3), L50_2 = round(EstL50_2, 3),
                              Delta_1 = round(EstDelta_1, 3), Delta_2 = round(EstDelta_2, 3),
                              Linf_F = round(EstLinf_F, 3), Linf_M = round(EstLinf_M, 3),
                              vbK_F = round(Estk_F, 3), vbK_M = round(Estk_M, 3), CV = round(EstCV, 3)))
    }
    if (SelectivityType == 3 & length(params)==8) { # selectivity vector input, separate sex input # logistic selectivity, separate sex input
      EstL50_1 = exp(c(nlmb$par[2], nlmb$par[2] + c(-1.96, 1.96) * ses[2]))
      EstL50_2 = exp(c(nlmb$par[3], nlmb$par[3] + c(-1.96, 1.96) * ses[3]))
      EstDelta_1 = exp(c(nlmb$par[4], nlmb$par[4] + c(-1.96, 1.96) * ses[4]))
      EstDelta_2 = exp(c(nlmb$par[5], nlmb$par[5] + c(-1.96, 1.96) * ses[5]))
      EstLinf = exp(c(nlmb$par[6], nlmb$par[6] + c(-1.96, 1.96) * ses[6]))
      Estk = exp(c(nlmb$par[7], nlmb$par[7] + c(-1.96, 1.96) * ses[7]))
      EstCV = exp(c(nlmb$par[8], nlmb$par[8] + c(-1.96, 1.96) * ses[8]))
      ParamEst = t(data.frame(FMort = round(EstFMort, 3), L50_1 = round(EstL50_1, 3), L50_2 = round(EstL50_2, 3),
                              Delta_1 = round(EstDelta_1, 3), Delta_2 = round(EstDelta_2, 3),
                              Linf = round(EstLinf, 3), vbK = round(Estk, 3), CV = round(EstCV, 3)))
...
#' @param SelectivityAtLen inputted selectivity at length vector
#' @param RetenParams params for fish retention
#' @param MLL minimum legal length
#'
#' @return SelAtLength, RetAtLength
GetSelectivityAndRetention <- function(midpt, SelectivityType, SelParams, SelectivityAtLen, RetenParams, MLL) {

  # gear selectivity
  if (SelectivityType > 1) { # logistic gear selectivity
    if (!is.na(SelParams[1])) { # calculate selectivity curve

      if (length(SelParams)==2) { # combined selectivity for sexes
        L50=SelParams[1]
        L95=L50 + SelParams[2]
        SelectivityAtLen = CalcLogisticSelOrReten(L50, L95, midpt)
      }
      if (length(SelParams)==4) { # separate selectivity for sexes
        L50=SelParams[1]
        L95=L50 + SelParams[3]
        SelectivityAtLen_Fem = CalcLogisticSelOrReten(L50, L95, midpt)
        L50=SelParams[2]
        L95=L50 + SelParams[4]
        SelectivityAtLen_Mal = CalcLogisticSelOrReten(L50, L95, midpt)
        SelectivityAtLen = t(data.frame(SelectivityAtLen_Fem=SelectivityAtLen_Fem,SelectivityAtLen_Mal=SelectivityAtLen_Mal))
        colnames(SelectivityAtLen) = midpt
      }
    }
  }
...
#' Get length transition matrices
#'
#' Get sex-specific length transition matrices
#'
#' @keywords internal
#'
#' @param GrowthCurveType 1=von Bertalanffy, 2=Schnute
#' @param GrowthParams growth parameters
#' @param RefnceAges reference ages for growth curves (Schnute)
#' @param MaxAge maximum age considered by model
#' @param TimeStep model timestep
#' @param lbnd lower bounds of length classes
#' @param midpt mid points of length classes
...
#' @param nLenCl number of length classes
#' @param SelectivityType 1=selectivity inputted as vector, 2=asymptotic logistic selectivity curve
#' @param CVSizeAtAge growth variation
#'
#' @return LTM_Fem, LTM_Mal
GetLengthTransitionMatrices <- function(GrowthCurveType, GrowthParams, RefnceAges, MaxAge, TimeStep, lbnd, midpt, ubnd, nLenCl,
                                        SelectivityType, CVSizeAtAge) {
...
#' Get expected discarded and retained catches for females, males and combined sexes
#'
#' Get expected discarded and retained catches for females, males and combined sexes, from rccp functions
#'
#' @keywords internal
#'
#' @param nTimeSteps number of model time steps
#' @param nLenCl number of length classes
#' @param midpt midpoints of length classes
#' @param CatchResults_Fem outputs from rcpp function to calculate expected equilibrium catches for females
#' @param CatchResults_Mal outputs from rcpp function to calculate expected equilibrium catches for males
#'
#' @return Results
GetExpCatchResults <- function(MaxAge, TimeStep, nTimeSteps, midpt, nLenCl, SelectivityType, SelParams, SelectivityAtLen, RetenParams, MLL,
                               NatMort, FishMort, DiscMort, InitRecNumber, RecLenDist, LTM_Fem, LTM_Mal) {

  FishMort_logit = log(FishMort/(1-FishMort)) # logit transform (so F is always between 0 and 1)

  # get selectivity and retention
  SelRes = GetSelectivityAndRetention(midpt, SelectivityType, SelParams, SelectivityAtLen, RetenParams, MLL)
  SelectivityAtLen = SelRes$SelectivityAtLen
  ...
  CatchResults_Fem = CalcCatches_AgeAndLengthBasedCatchCurves_cpp(FishMort_logit, NatMort, RecLenDist_Fem, InitRecNumber, MaxAge, TimeStep, nTimeSteps,
                                                                  nLenCl, midpt, RetAtLength, SelAtLength, DiscMort, LTM_Fem)
  if (is.vector(SelectivityAtLen)) { # single selectivity curve for combined sexes
    SelAtLength = as.vector(unlist(SelectivityAtLen))
  } else { # separate sex
    SelAtLength = as.vector(unlist(SelectivityAtLen[2,]))
  }
  ...
  RetCatchAtLen_Fem = CatchResults_Fem$RetCatchAtLen; RetCatchAtLen_Mal = CatchResults_Mal$RetCatchAtLen
  RetCatchAtLen = RetCatchAtLen_Fem + RetCatchAtLen_Mal

  # expected discarded catch at length
  DiscCatchAtLen_Fem = CatchResults_Fem$DiscCatchAtLen; DiscCatchAtLen_Mal = CatchResults_Mal$DiscCatchAtLen
  DiscCatchAtLen = DiscCatchAtLen_Fem + DiscCatchAtLen_Mal

  # expected total catch at length (discarded pus retained catches)
  TotCatchAtLen_Fem = CatchResults_Fem$TotCatchAtLen; TotCatchAtLen_Mal = CatchResults_Mal$TotCatchAtLen
  TotCatchAtLen = TotCatchAtLen_Fem + TotCatchAtLen_Mal

  # expected retained catch at decimal age and length
  RetCatchAtDecAgeLen_Fem = as.matrix(unlist(CatchResults_Fem$RetCatchAtDecAgeLen)) # retained catches, decimal ages and lengths
  RetCatchAtDecAgeLen_Mal = as.matrix(unlist(CatchResults_Mal$RetCatchAtDecAgeLen))
  ...
  DiscCatchAtDecAgeLen_Fem = as.matrix(unlist(CatchResults_Fem$DiscCatchAtDecAgeLen)) # retained catches, decimal ages and lengths
  DiscCatchAtDecAgeLen_Mal = as.matrix(unlist(CatchResults_Mal$DiscCatchAtDecAgeLen))
  ...
  TotCatchAtDecAgeLen_Fem = as.matrix(unlist(CatchResults_Fem$TotCatchAtDecAgeLen)) # retained catches, decimal ages and lengths
  TotCatchAtDecAgeLen_Mal = as.matrix(unlist(CatchResults_Mal$TotCatchAtDecAgeLen))
  ...
  Results = list(SelAtLength = SelAtLength,
                 ...
                 FAtLenCapt = CatchResults_Fem$FAtLenCapt,
                 FAtLenReten = CatchResults_Fem$FAtLenReten,
                 FAtLenDisc = CatchResults_Fem$FAtLenDisc,
                 RetCatchAtLen_Fem=RetCatchAtLen_Fem,
                 RetCatchAtLen_Mal=RetCatchAtLen_Mal,
                 RetCatchAtLen=RetCatchAtLen,
                 DiscCatchAtLen_Fem=DiscCatchAtLen_Fem,
                 DiscCatchAtLen_Mal=DiscCatchAtLen_Mal,
                 DiscCatchAtLen=DiscCatchAtLen,
                 TotCatchAtLen_Fem=TotCatchAtLen_Fem,
                 TotCatchAtLen_Mal=TotCatchAtLen_Mal,
                 TotCatchAtLen=TotCatchAtLen,
                 RetCatchAtDecAgeLen_Fem=RetCatchAtDecAgeLen_Fem,
                 RetCatchAtDecAgeLen_Mal=RetCatchAtDecAgeLen_Mal,
                 RetCatchAtDecAgeLen=RetCatchAtDecAgeLen,
                 DiscCatchAtDecAgeLen_Fem=DiscCatchAtDecAgeLen_Fem,
                 DiscCatchAtDecAgeLen_Mal=DiscCatchAtDecAgeLen_Mal,
                 DiscCatchAtDecAgeLen=DiscCatchAtDecAgeLen,
                 TotCatchAtDecAgeLen_Fem=TotCatchAtDecAgeLen_Fem,
                 TotCatchAtDecAgeLen_Mal=TotCatchAtDecAgeLen_Mal,
                 TotCatchAtDecAgeLen=TotCatchAtDecAgeLen,
                 ...
                 ExpDiscCatchPropAtDecAge=ExpDiscCatchPropAtDecAge,
...
#' @param SelectivityAtLen selectivity at length, set to NA if selectivity parameters inputted
#' @param DiscMort Proportion of fish that die following to capture and release
#' @param GrowthCurveType 1=von Bertalanffy, 2=Schnute
...
#'                             SelParams, RetenParams, SelectivityAtLen, DiscMort, GrowthCurveType, GrowthParams, RefnceAges, CVSizeAtAge)
#' @export
SimLenAndAgeFreqData_EqMod <- function(SampleSize, MaxAge, TimeStep, NatMort, FishMort, MaxLen, LenInc, MLL, SelectivityType,
                                 SelParams, RetenParams, SelectivityAtLen, DiscMort, GrowthCurveType, GrowthParams, RefnceAges, CVSizeAtAge) {
  ...
  # Get sex-specific length transition matrices
  Res = GetLengthTransitionMatrices(GrowthCurveType, GrowthParams, RefnceAges, MaxAge, TimeStep, lbnd, midpt, ubnd, nLenCl,
                                    SelectivityType, CVSizeAtAge)
  LTM_Fem = Res$LTM_Fem; LTM_Mal = Res$LTM_Mal
  MeanSizeAtAge = Res$MeanSizeAtAge
  ...
  # get expected catches required to generate random composition data
  ExpCatchRes = GetExpCatchResults(MaxAge, TimeStep, nTimeSteps, midpt, nLenCl, SelectivityType, SelParams, SelectivityAtLen, RetenParams, MLL,
                                 NatMort, FishMort, DiscMort, InitRecNumber, RecLenDist, LTM_Fem, LTM_Mal)
  RetCatchAtLen_Fem = ExpCatchRes$RetCatchAtLen_Fem; RetCatchAtLen_Mal = ExpCatchRes$RetCatchAtLen_Mal
  DiscCatchAtLen_Fem = ExpCatchRes$DiscCatchAtLen_Fem; DiscCatchAtLen_Mal = ExpCatchRes$DiscCatchAtLen_Mal
  ExpRetCatchPropAtLen_Fem = ExpCatchRes$ExpRetCatchPropAtLen_Fem; ExpRetCatchPropAtLen_Mal = ExpCatchRes$ExpRetCatchPropAtLen_Mal
  ExpDiscCatchPropAtLen_Fem = ExpCatchRes$ExpDiscCatchPropAtLen_Fem; ExpDiscCatchPropAtLen_Mal = ExpCatchRes$ExpDiscCatchPropAtLen_Mal
  ExpRetCatchPropDecAgeGivenLength_Fem = ExpCatchRes$ExpRetCatchPropDecAgeGivenLength_Fem; ExpRetCatchPropDecAgeGivenLength_Mal = ExpCatchRes$ExpRetCatchPropDecAgeGivenLength_Mal
  ExpDiscCatchPropDecAgeGivenLength_Fem = ExpCatchRes$ExpDiscCatchPropDecAgeGivenLength_Fem; ExpDiscCatchPropDecAgeGivenLength_Mal = ExpCatchRes$ExpDiscCatchPropDecAgeGivenLength_Mal
  ...
  ModelDiag = list(DecAges = DecAges,
                   ...
                   RecLenDist = RecLenDist,
                   MeanSizeAtAge = MeanSizeAtAge,
                   SelAtLength=ExpCatchRes$SelAtLength,
                   RetAtLength=ExpCatchRes$RetAtLength,
                   SelLandAtLength=ExpCatchRes$SelLandAtLength,
                   SelDiscAtLength=ExpCatchRes$SelDiscAtLength,
                   FAtLen = ExpCatchRes$FAtLen,
                   ZAtLen = ExpCatchRes$ZAtLen,
                   FAtLenCapt = ExpCatchRes$FAtLenCapt,
                   ...
                   DiscCatchAtDecAge_Fem = ExpCatchRes$DiscCatchAtDecAge_Fem,
                   DiscCatchAtDecAge_Mal = ExpCatchRes$DiscCatchAtDecAge_Mal,
                   DiscCatchAtDecAge = ExpCatchRes$DiscCatchAtDecAge,
                   TotCatchAtDecAge_Fem = ExpCatchRes$TotCatchAtDecAge_Fem,
                   TotCatchAtDecAge_Mal = ExpCatchRes$TotCatchAtDecAge_Mal,
                   TotCatchAtDecAge = ExpCatchRes$TotCatchAtDecAge,
                   RetCatchAtLen_Fem = ExpCatchRes$RetCatchAtLen_Fem,
                   RetCatchAtLen_Mal = ExpCatchRes$RetCatchAtLen_Mal,
                   DiscCatchAtLen_Fem = ExpCatchRes$DiscCatchAtLen_Fem,
                   DiscCatchAtLen_Mal = ExpCatchRes$DiscCatchAtLen_Mal,
                   ExpRetCatchPropAtLen_Fem = ExpCatchRes$ExpRetCatchPropAtLen_Fem,
                   ExpRetCatchPropAtLen_Mal = ExpCatchRes$ExpRetCatchPropAtLen_Mal,
                   ExpDiscCatchPropAtLen_Fem = ExpCatchRes$ExpDiscCatchPropAtLen_Fem,
                   ExpDiscCatchPropAtLen_Mal = ExpCatchRes$ExpDiscCatchPropAtLen_Mal,
                   ExpRetCatchPropDecAgeGivenLength_Fem = ExpCatchRes$ExpRetCatchPropDecAgeGivenLength_Fem,
                   ExpRetCatchPropDecAgeGivenLength_Mal = ExpCatchRes$ExpRetCatchPropDecAgeGivenLength_Mal,
                   ExpDiscCatchPropDecAgeGivenLength_Fem = ExpCatchRes$ExpDiscCatchPropDecAgeGivenLength_Fem,
                   ExpDiscCatchPropDecAgeGivenLength_Mal = ExpCatchRes$ExpDiscCatchPropDecAgeGivenLength_Mal,
...
#' Update population dynamics for age-based dynamic simulation model
#'
#' Generates outputs from a dynamic simulation model for generation of
#' annual age composition data
#'
#' @keywords internal
#'
#' @param nYears number of years of deviations required
...
#' @param PropFemAtAge proportion females at age
#' @param FemPropMatAtAge female proportion mature at age
#' @param MalPropMatAtAge male proportion mature at age
#' @param FemWtAtAge female weight at age
#' @param MalWtAtAge male weight at age
#' @param FishFemSurvAtAge numbers of females per recruit in each age class
#' @param FishMalSurvAtAge numbers of males per recruit in each age class
...
#' @param FemSelLandAtAge selectivity of landings for females
#' @param MalSelLandAtAge selectivity of landings for males
#' @param FemSelDiscAtAge selectivity of discards for females
#' @param MalSelDiscAtAge selectivity of discards for males
#'
#' @return FemSpBiom, MalSpBiom, CombSexSpBiom, AnnRecruit, Catch_Fem, Catch_Mal, Catch_CombSex, Catch_Biom
UpdatePopnDynamics_DynSimMod_AB <- function(nYears, MaxModelAge, NatMort, ReprodPattern, InitRatioFem, PropFemAtAge,
                                            FemPropMatAtAge, MalPropMatAtAge, FemWtAtAge, MalWtAtAge,
                                            FishFemSurvAtAge, FishMalSurvAtAge, Fish_FemSpBiomPerRec,
                                            Fish_CombSexSpBiomPerRec, BH_SRRa, BH_SRRb, random_dev, FMortByYear, DiscMort,
                                            FemSelLandAtAge, MalSelLandAtAge, FemSelDiscAtAge, MalSelDiscAtAge) {

  Ages <- seq(0,MaxModelAge,1)
  nAges <- length(Ages)
  N_Fem <- data.frame(matrix(nrow = nYears+1, ncol = nAges)) # numbers
  colnames(N_Fem) <- Ages
  N_Fem[seq(1,nYears+1,1),seq(1,nAges,1)] = 0
  N_Mal = N_Fem
  tempSurv_Fem = N_Fem; tempSurv_Mal = N_Fem
  Catch_Fem <- data.frame(matrix(nrow = nYears, ncol = nAges)) # numbers
  colnames(Catch_Fem) <- Ages
  Catch_Fem[seq(1,nAges,1),seq(1,nAges,1)] = 0
  Catch_Mal = Catch_Fem; Catch_CombSex = N_Fem
  ...
  for (t in 1:nYears) {
    ...
    if (t == 1) {
      ...
      N_Fem[t,] = AnnRecruit[t] * FishFemSurvAtAge # 1000s
      N_Mal[t,] = AnnRecruit[t] * FishMalSurvAtAge # 1000s
    }

    # get fishing and total mortality at length (for calculating survival)
    FemFAtAge <- FMortByYear[t] * (FemSelLandAtAge + (DiscMort * FemSelDiscAtAge))
    MalFAtAge <- FMortByYear[t] * (MalSelLandAtAge + (DiscMort * MalSelDiscAtAge))
    FemZAtAge = NatMort + FemFAtAge
    MalZAtAge = NatMort + MalFAtAge

    # calculate survival from ages 1 to MaxModelAge
    for (a in seq(1,nAges-1,1)) {
      if (a < nAges-1) {

        N_Fem[t+1,a+1] = N_Fem[t,a] * exp(-FemZAtAge[a])
        N_Mal[t+1,a+1] = N_Mal[t,a] * exp(-MalZAtAge[a])

      } else {

        N_Fem[t+1,a+1] = N_Fem[t,a] * exp(-FemZAtAge[a]) / (1 - exp(-FemZAtAge[a]))
        N_Mal[t+1,a+1] = N_Mal[t,a] * exp(-MalZAtAge[a]) / (1 - exp(-MalZAtAge[a]))
      } # else
    } # a
    ...
    if (ReprodPattern > 1) { # hermaphroditic species
      ...
      for (a in seq(2,nAges-1,1)) {
        tempSurv_Fem[t+1,a] = PropFemAtAge[a] * (N_Fem[t+1,a] + N_Mal[t+1,a])
        tempSurv_Mal[t+1,a] = (1 - PropFemAtAge[a]) * (N_Fem[t+1,a] + N_Mal[t+1,a])
        N_Fem[t+1,a] = tempSurv_Fem[t+1,a]
        N_Mal[t+1,a] = tempSurv_Mal[t+1,a]

      } # a
    } # reproductive pattern

    # calculate spawning biomass
    FemSpBiom[t] = sum(N_Fem[t,] * FemPropMatAtAge * FemWtAtAge) # tonnes
    MalSpBiom[t] = sum(N_Mal[t,] * MalPropMatAtAge * MalWtAtAge) # tonnes
    ...
    # add 0+ recruits
    N_Fem[t+1,1] = AnnRecruit[t+1] * InitRatioFem
    N_Mal[t+1,1] = AnnRecruit[t+1] * (1 - InitRatioFem)

    # calculate female and male fishing mortality at age associated with landings
    FemLandFAtAge = FMortByYear[t] * FemSelLandAtAge
    MalLandFAtAge = FMortByYear[t] * MalSelLandAtAge

    # calculate retained catch at length for timestep (in numbers)
    Catch_Fem[t,] = N_Fem[t,] * (FemLandFAtAge / FemZAtAge) * (1 - exp(-FemZAtAge))
    Catch_Mal[t,] = N_Mal[t,] * (MalLandFAtAge / MalZAtAge) * (1 - exp(-MalZAtAge))
    Catch_CombSex[t,] = Catch_Fem[t,] + Catch_Mal[t,]
    TotCatch[t] = sum(Catch_Fem[t,]) + sum(Catch_Mal[t,])

    # calculate retained catch biomass
    Catch_Biom[t] = sum(Catch_Fem[t,] * FemWtAtAge) + sum(Catch_Mal[t,] * MalWtAtAge)
...
#' Update population dynamics for length-based dynamic simulation model
#'
#' Generates outputs from a dynamic simulation model for generation of
...
#' @param PropFemAtLen proportion females at length
...
#'
#' @return FemSpBiom, MalSpBiom, CombSexSpBiom, AnnRecruit, RetCatch_Fem, RetCatch_Mal, RetCatch_CombSex, RetCatch_Biom,
#' DiscCatch_Fem, DiscCatch_Mal, DiscCatch_CombSex, DiscCatch_Biom
UpdatePopnDynamics_DynSimMod_LB <- function(nYears, nLenCl, midpt, NatMort, ReprodPattern, InitRatioFem, PropFemAtLen, RecLenDist,
  ...
  RetCatch_Fem <- data.frame(matrix(nrow = nYears, ncol = nLenCl)) # numbers
  colnames(RetCatch_Fem) <- midpt
  RetCatch_Fem[seq(1,nYears,1),seq(1,nLenCl,1)] = 0
  RetCatch_Mal = RetCatch_Fem; RetCatch_CombSex = RetCatch_Fem
  DiscCatch_Fem = RetCatch_Fem; DiscCatch_Mal = RetCatch_Fem; DiscCatch_CombSex = RetCatch_Fem
  ...
  RetCatch_Biom = rep(0,nYears); DiscCatch_Biom = rep(0,nYears)
  ...
  for (t in 1:nYears) {
    ...
    FemZAtLen = NatMort + FemFAtLen
    Surv_Fem[t+1,] = N_Fem[t,] * exp(-FemZAtLen)
    MalZAtLen = NatMort + MalFAtLen
    Surv_Mal[t+1,] = N_Mal[t,] * exp(-MalZAtLen)
    ...
    # calculate retained catch at length for timestep (in numbers)
    RetCatch_Fem[t,] = N_Fem[t,] * (FemLandFAtLen / FemZAtLen) * (1 - exp(-FemZAtLen))
    RetCatch_Mal[t,] = N_Mal[t,] * (MalLandFAtLen / MalZAtLen) * (1 - exp(-MalZAtLen))
    RetCatch_CombSex[t,] = RetCatch_Fem[t,] + RetCatch_Mal[t,]
    RetCatch_Num[t] = sum(RetCatch_CombSex[t,])

    # calculate female and male fishing 'level' at length associated with discarding
    # to generate size distributions for discarded catches (whether they survive or not),
    # i.e. not considering proportion of these that die.
    FemDiscCatchFAtLen = FMortByYear[t] * FemSelDiscAtLen
    MalDiscCatchFAtLen = FMortByYear[t] * MalSelDiscAtLen

    DiscCatch_Fem[t,] = N_Fem[t,] * (FemDiscCatchFAtLen / FemZAtLen) * (1 - exp(-FemZAtLen))
    DiscCatch_Mal[t,] = N_Mal[t,] * (MalDiscCatchFAtLen / MalZAtLen) * (1 - exp(-MalZAtLen))
    DiscCatch_CombSex[t,] = DiscCatch_Fem[t,] + DiscCatch_Mal[t,]
    DiscCatch_Num[t] = sum(DiscCatch_CombSex[t,])

    # calculate retained and discarded catch biomass
    RetCatch_Biom[t] = sum(RetCatch_Fem[t,] * FemWtAtLen) + sum(RetCatch_Mal[t,] * MalWtAtLen)
    DiscCatch_Biom[t] = sum(DiscCatch_Fem[t,] * FemWtAtLen) + sum(DiscCatch_Mal[t,] * MalWtAtLen)
  ...
  Results = list(FemSpBiom=FemSpBiom,
                 ...
                 RetCatch_Fem=RetCatch_Fem,
                 RetCatch_Mal=RetCatch_Mal,
                 RetCatch_CombSex=RetCatch_CombSex,
                 RetCatch_Biom=RetCatch_Biom,
                 DiscCatch_Num=DiscCatch_Num,
                 DiscCatch_Fem=DiscCatch_Fem,
                 DiscCatch_Mal=DiscCatch_Mal,
                 DiscCatch_CombSex=DiscCatch_CombSex,
...
#' model (using SimLenFreqData_DynMod_LB). Values of fishing mortality are generated
...
#' Generate random age data from age-based dynamic simulation model
#'
...
#' @keywords internal
#'
#' @param nYears number of years of age data
...
#' @param Catch_CombSex number of females and males in annual retained catches in each length class
#'
#' @return MeanLengthStats
GenerateRandomAgeFreqData_DynSimMod <- function(nYears, SimAnnSampSize, MaxModelAge, Catch_Fem, Catch_Mal, Catch_CombSex) {

  Ages <- seq(0,MaxModelAge,1)
  nAges <- length(Ages)

  # Get sample random length frequency data, for retained catches for each year,
  # given expected age distributions for each year, assuming a multinomial distribution
  AnnSimSampSize_Fem = rep(0, nYears); AnnSimSampSize_Mal = rep(0, nYears)
  RandObsCatchAgeFreq_Fem <- data.frame(matrix(nrow = nYears+1, ncol = nAges))
  colnames(RandObsCatchAgeFreq_Fem) <- Ages
  RandObsCatchAgeFreq_Fem[seq(1,nYears+1,1),seq(1,nAges,1)] = 0
  RandObsCatchAgeFreq_Mal = RandObsCatchAgeFreq_Fem; RandObsCatchAgeFreq_CombSex = RandObsCatchAgeFreq_Fem
  ...
  for (t in 1:nYears) {
    ...
    RandObsCatchAgeFreq_Fem[t,] = as.vector(rmultinom(1, AnnSimSampSize_Fem[t], Probs_Fem))
    RandObsCatchAgeFreq_Mal[t,] = as.vector(rmultinom(1, AnnSimSampSize_Mal[t], Probs_Mal))
    RandObsCatchAgeFreq_CombSex[t,] = RandObsCatchAgeFreq_Fem[t,] + RandObsCatchAgeFreq_Mal[t,]
  }

  results = list(RandObsCatchAgeFreq_Fem = RandObsCatchAgeFreq_Fem,
                 RandObsCatchAgeFreq_Mal = RandObsCatchAgeFreq_Mal,
                 RandObsCatchAgeFreq_CombSex = RandObsCatchAgeFreq_CombSex,
...
#' Generate random length data from length-based dynamic simulation model
#'
#' Generates random length data, and associated length frequencies,
#' for each year, from dynamic length-based simulation model
#'
#' @keywords internal
#'
...
GenerateRandomLengthFreqData_DynSimMod <- function(nYears, SimAnnSampSize, lbnd, midpt, ubnd, Catch_Fem, Catch_Mal, Catch_CombSex) {
  ...
  for (t in 1:nYears) {
    ...
    Probs_Fem = as.vector(unlist(Catch_Fem[t,] / sum(Catch_Fem[t,])))
    Probs_Mal = as.vector(unlist(Catch_Mal[t,] / sum(Catch_Mal[t,])))
    AnnSimSampSize_Fem[t] = round(sum(Catch_Fem[t,]) / sum(Catch_CombSex[t,]) * SimAnnSampSize[t], 0)
    AnnSimSampSize_Mal[t] = SimAnnSampSize[t] - AnnSimSampSize_Fem[t]

    if (AnnSimSampSize_Fem[t] > 0) {
      RandObsCatchLenFreq_Fem[t,] = as.vector(rmultinom(1, AnnSimSampSize_Fem[t], Probs_Fem))
    }
    if (AnnSimSampSize_Mal[t] > 0) {
...
#' Calculate mean age statistics for dynamic simulation model
#'
#' Calculates mean ages and associated 95 percent confidence limits for
#' each year of generated age data from dynamic simulation model
#'
#' @keywords internal
#'
#' @param nYears number of years of length data
...
#' @param EstLenAtAge input vector length at age
...
#' @param EstWtAtAge weight at ages
...
#' @param EstSexRatioAtAge NA  # sex ratio at age, inputted as vector
...
#' @param mat_A50 logistic age at maturity parameter (set to NA if directly inputting proportion mature at age)
#' @param mat_A95 logistic age at maturity parameter (set to NA if directly inputting proportion mature at age)
...
#' @param EstGearSelAtAge gear selectivity curve inputted as vector
...
#' @param EstRetenAtAge retention curve inputted as vector
#' @param DiscMort proportion of fish that die following capture and release
...
#' @param NatMort natural mortality
#' @param FMortByYear annual fully-selected fishing mortality
#'
...
#' @export
SimAgeFreqData_DynMod_AB <- function(SimAnnSampSize, nYears, lnSigmaR, autocorr, InitRec, MaxModelAge, TimeStep, Linf, vbK, tzero, EstLenAtAge,
                                     lenwt_a, ln_lenwt_a, lenwt_b, WLrel_Type, EstWtAtAge, ReprodScale,
                                     ReprodPattern, InitRatioFem, FinalSex_A50, FinalSex_A95, EstSexRatioAtAge,
                                     EggFertParam, mat_A50, mat_A95, EstMatAtAge, Gear_sel_A50, Gear_sel_A95,
                                     EstGearSelAtAge, Land_sel_A50, Land_sel_A95, EstLandSelAtAge, ret_Pmax,
  ...
  Fish_CombSexSpBiomPerRec = sum(res$ModelDiag$FishFemSpBiomAtAge) + sum(res$ModelDiag$FishMalSpBiomAtAge)
  FemGearSelAtAge = res$ModelDiag$FemGearSelAtAge; MalGearSelAtAge = res$ModelDiag$MalGearSelAtAge
  FemRetProbAtAge = res$ModelDiag$FemRetProbAtAge; MalRetProbAtAge = res$ModelDiag$MalRetProbAtAge
  FemSelLandAtAge = res$ModelDiag$FemSelLandAtAge; MalSelLandAtAge = res$ModelDiag$MalSelLandAtAge
  FemSelDiscAtAge = res$ModelDiag$FemSelDiscAtAge; MalSelDiscAtAge = res$ModelDiag$MalSelDiscAtAge
  FemPropMatAtAge = res$ModelDiag$FemPropMatAtAge; MalPropMatAtAge = res$ModelDiag$MalPropMatAtAge
  PropFemAtAge = res$ModelDiag$PropFemAtAge
  ...
  res=UpdatePopnDynamics_DynSimMod_AB(nYears, MaxModelAge, NatMort, ReprodPattern, InitRatioFem, PropFemAtAge,
                                      FemPropMatAtAge, MalPropMatAtAge, FemWtAtAge, MalWtAtAge,
                                      FishFemSurvAtAge, FishMalSurvAtAge, Fish_FemSpBiomPerRec,
                                      Fish_CombSexSpBiomPerRec, BH_SRRa, BH_SRRb, random_dev, FMortByYear, DiscMort,
                                      FemSelLandAtAge, MalSelLandAtAge, FemSelDiscAtAge, MalSelDiscAtAge)
...
#' @export
SimLenFreqData_DynMod_LB <- function(SimAnnSampSize, nYears, lnSigmaR, autocorr, InitRec, MaxModelAge, TimeStep, lbnd, ubnd, midpt, nLenCl,
  ...
  # update population dynamics, and calculate annual catches
  res=UpdatePopnDynamics_DynSimMod_LB(nYears, nLenCl, midpt, NatMort, ReprodPattern, InitRatioFem, PropFemAtLen, RecLenDist,
                                   FemPropMatAtLen, MalPropMatAtLen, FemWtAtLen, MalWtAtLen, LTM_Fem, LTM_Mal,
                                   Fish_FemNPerRecAtLen, Fish_MalNPerRecAtLen, Fish_FemSpBiomPerRec,
                                   Fish_CombSexSpBiomPerRec, BH_SRRa, BH_SRRb, random_dev, FMortByYear, DiscMort,
                                   FemSelLandAtLen, MalSelLandAtLen, FemSelDiscAtLen, MalSelDiscAtLen)
  ...
  FemSpBiom=res$FemSpBiom; MalSpBiom=res$MalSpBiom; CombSexSpBiom=res$CombSexSpBiom; AnnRecruit=res$AnnRecruit
  RetCatch_Fem=res$RetCatch_Fem; RetCatch_Mal=res$RetCatch_Mal; RetCatch_CombSex=res$RetCatch_CombSex; RetCatch_Biom=res$RetCatch_Biom
  DiscCatch_Fem=res$DiscCatch_Fem; DiscCatch_Mal=res$DiscCatch_Mal; DiscCatch_CombSex=res$DiscCatch_CombSex; DiscCatch_Biom=res$DiscCatch_Biom
  RetCatch_Num=res$RetCatch_Num; DiscCatch_Num=res$DiscCatch_Num
  ...
  RetCatchMeanLengthStats = GetMeanLengthStats_DynSimMod(nYears, RandRetCatchLenFreq_Fem, RandRetCatchLenFreq_Mal)
  ...
  res=list(RetCatch_Fem = RetCatch_Fem,
           ...
           AnnRecruit = AnnRecruit,
           random_dev = random_dev,
           AnnRetCatchSimSampSize_Fem = AnnRetCatchSimSampSize_Fem,
           AnnRetCatchSimSampSize_Mal = AnnRetCatchSimSampSize_Mal,
           RandRetCatchLenFreq_Fem = RandRetCatchLenFreq_Fem,
           RandRetCatchLenFreq_Mal = RandRetCatchLenFreq_Mal,
           RandRetCatchLenFreq_CombSex = RandRetCatchLenFreq_CombSex,
           RetCatchMeanLengthStats = RetCatchMeanLengthStats,
           AnnDiscCatchSimSampSize_Fem = AnnDiscCatchSimSampSize_Fem,
           AnnDiscCatchSimSampSize_Mal = AnnDiscCatchSimSampSize_Mal,
           RandDiscCatchLenFreq_Fem = RandDiscCatchLenFreq_Fem,
           RandDiscCatchLenFreq_Mal = RandDiscCatchLenFreq_Mal,
           RandDiscCatchLenFreq_CombSex = RandDiscCatchLenFreq_CombSex,
           DiscCatchMeanLengthStats = DiscCatchMeanLengthStats)
...
#' @param MaxAge Maximum age
#' @param MaxLen Maximum length
#' @param SimRes Outputs of SimLenAndAgeFreqData_EqMod function
...
PlotSimLenAndAgeFreqData <- function(MaxAge, MaxLen, SimRes, PlotOpt) {
  ...
  if (PlotOpt==0 | PlotOpt == 11) {
    ...
    yint = 0.2
    # gear selectivity
    if (is.vector(SimRes$ModelDiag$SelectivityAtLen)) {
      plot(SimRes$midpt, SimRes$ModelDiag$SelectivityAtLen, "l", cex.main=1.2, pch=1, cex=0.6,
           xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=list(yaxis_lab,cex=1.2), frame=F, xlim=c(0,xmax),
           ylim=c(0,ymax), col="red")
      lines(SimRes$midpt, SimRes$ModelDiag$SelLandAtLength, col="black") # same for both sexes
      if (SelectivityType == 1) {
      ...
      plot(SimRes$midpt, SimRes$ModelDiag$SelectivityAtLen[1,], "l", cex.main=1.2, pch=1, cex=0.6,
           xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=list(yaxis_lab,cex=1.2), frame=F, xlim=c(0,xmax),
           ylim=c(0,ymax), col="red")
      lines(SimRes$midpt, SimRes$ModelDiag$SelectivityAtLen[2,], col="purple")
      lines(SimRes$midpt, SimRes$ModelDiag$SelLandAtLength, col="black") # same for both sexes
      if (SelectivityType == 1) {
        legend("bottomright", pch=-1, legend=c("Sel_gear_Fem", "Sel_gear_Mal", "Prob_Reten", "Sel_land"),
               lty="solid", col=c("red","purple","blue","black"), bty='n', cex=0.8,lwd=1, y.intersp=1.1)
      }
    }
  ...
  if (PlotOpt==0 | PlotOpt == 12) {
    ...
    xmax = xlims$xmax
    xint = xlims$xint
    ylims = Get_yaxis_scale(c(0,SimRes$ModelDiag$FAtLen))
    ymax = ylims$ymax
    yint = ylims$yint
    yaxis_lab = expression(paste(italic("F") ~ (year^{-1})))
    xaxis_lab = "Length (mm)"
    plot(SimRes$midpt, SimRes$ModelDiag$FAtLen, "l", main="F (retention + discard mortality)", cex.main=1.0, pch=1, cex=0.6,
  ...
  if (PlotOpt==0 | PlotOpt == 13) {
    ...
    yaxis_lab = expression(paste(italic("F") ~ (year^{-1})))
    xaxis_lab = "Length (mm)"
    plot(SimRes$midpt, SimRes$ModelDiag$FAtLenReten, "l", main="F (retention)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
         ylim=c(0,ymax), col="red")
    AddAxesAndTickLabelsToPlot(xmin=NA, xmax, xint, ymin=NA, ymax, yint, cexval=NA, cexaxisval=NA, lwdval=NA, lineval=NA, lasval=NA)
  ...
  if (PlotOpt==0 | PlotOpt == 14) {
    ...
    yaxis_lab = expression(paste(italic("F") ~ (year^{-1})))
    xaxis_lab = "Length (mm)"
    plot(SimRes$midpt, SimRes$ModelDiag$FAtLenDisc*DiscMort, "l", main="F (discard mortality)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
         ylim=c(0,ymax), col="red")
    AddAxesAndTickLabelsToPlot(xmin=NA, xmax, xint, ymin=NA, ymax, yint, cexval=NA, cexaxisval=NA, lwdval=NA, lineval=NA, lasval=NA)
    mtext(expression(paste(italic("F") ~ (year^{-1}))),las=3,side=2,line=2.5,cex=1.0,lwd=1.75)
...
PlotLengthBasedCatchCurve_RetCatch <- function(params, DistnType, MLL, SelectivityType, ObsRetCatchFreqAtLen, lbnd, ubnd, midpt,
                                             SelectivityAtLen, PropReleased, ObsDiscCatchFreqAtLen, DiscMort, GrowthCurveType, GrowthParams, RefnceAges,
...
#'                                     SelectivityAtLen, PropReleased, ObsDiscCatchFreqAtLen, DiscMort, GrowthCurveType, GrowthParams, RefnceAges, MaxAge, NatMort, TimeStep, MainLabel=NA,
#'                                     xaxis_lab=NA, yaxis_lab=NA, xmax=1500, xint=500,
#'                                     ymax=0.15, yint=0.05, PlotCLs=TRUE, FittedRes, nReps=200)
#' @export
...
#' @export
PlotLengthBasedCatchCurve_Selectivity <- function(params, DistnType, MLL, SelectivityType, ObsRetCatchFreqAtLen, lbnd, ubnd, midpt, SelectivityAtLen,
                                                  PropReleased, ObsDiscCatchFreqAtLen, DiscMort, GrowthCurveType, GrowthParams, RefnceAges,
                                                  MaxAge, NatMort, TimeStep, MainLabel, xaxis_lab, yaxis_lab, xmax, xint,
                                                  ymax, yint, PlotCLs, FittedRes, nReps) {
  ...
  if (SelectivityType == 1) { # inputted selectivity at length values
    if (is.vector(SelectivityAtLen)) { # single selectivity curve for combined sexes
      plot(midpt, SelectivityAtLen, "l", main=MainLabel, cex.main=1.2, pch=1, cex=0.6,
           xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=list(yaxis_lab,cex=1.2), frame=F, xlim=c(0,xmax),
           ylim=c(0,ymax), col="red")
      lines(midpt, res$ModelDiag$RetAtLength, col="blue")
      lines(midpt, res$ModelDiag$SelLandAtLength[1,], col="black")
      legend("bottomright", pch=-1, legend=c("Sel_gear", "Prob_Reten", "Sel_land"), lty="solid", col=c("red","blue","black"),
             bty='n', cex=0.8,lwd=1, y.intersp=1.1)
    } else { # separate sex
      plot(midpt, SelectivityAtLen[1,], "l", main=MainLabel, cex.main=1.2, pch=1, cex=0.6,
           xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=list(yaxis_lab,cex=1.2), frame=F, xlim=c(0,xmax),
           ylim=c(0,ymax), col="red")
      lines(midpt, res$ModelDiag$RetAtLength, col="blue")
      lines(midpt, SelectivityAtLen[2,], col="purple")
      lines(midpt, res$ModelDiag$SelLandAtLength[1,], col="black")
      lines(midpt, res$ModelDiag$SelLandAtLength[2,], col="grey")
      legend("bottomright", pch=-1, legend=c("Sel_gear_Fem", "Sel_gear_Mal", "Prob_Reten", "Sel_land_Fem", "Sel_land_Mal"),
             lty="solid", col=c("red","purple","blue","black","grey"), bty='n', cex=0.8,lwd=1, y.intersp=1.1)
    }
    AddAxesAndTickLabelsToPlot(xmin=0, xmax, xint, ymin=0, ymax, yint, cexval=NA, cexaxisval=1, lwdval=0, lineval=0, lasval=1)
  } # SelectivityType == 1
...
#' @export
PlotLengthBasedCatchCurve_Mortality <- function(params, DistnType, MLL, SelectivityType, ObsRetCatchFreqAtLen, lbnd, ubnd, midpt, SelectivityAtLen,
                                                PropReleased, ObsDiscCatchFreqAtLen, DiscMort, GrowthCurveType, GrowthParams, RefnceAges,
  ...
  if (GrowthModelType ==  2 | GrowthModelType == 4) { # separate sex
    ylims = Get_yaxis_scale(c(res$FAtLen_Fem,res$FAtLen_Mal))
    if (is.na(ymax)) ymax = ylims$ymax
    if (is.na(yint)) yint = ylims$yint
    plot(midpt, res$FAtLen_Fem, "l", main="F (retention + discard mortality)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
         ylim=c(0,ymax), col="red")
    AddAxesAndTickLabelsToPlot(xmin=0, xmax, xint, ymin=0, ymax, yint, cexval=NA, cexaxisval=1, lwdval=1.75, lineval=-0.2, lasval=1)
    mtext(expression(paste(italic("F") ~ (year^{-1}))),las=3,side=2,line=2.5,cex=1.0,lwd=1.75)
  ...
  # F at age, associated with retention
  if (GrowthModelType ==  1 | GrowthModelType == 3) { # single sex
    plot(midpt, res$FAtLenReten, "l", main="F (retention)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
  ...
  if (GrowthModelType ==  2 | GrowthModelType == 4) { # separate sex
    plot(midpt, res$FAtLenReten_Fem, "l", main="F (retention)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
         ylim=c(0,ymax), col="red")
    AddAxesAndTickLabelsToPlot(xmin=0, xmax, xint, ymin=0, ymax, yint, cexval=NA, cexaxisval=1, lwdval=1.75, lineval=-0.2, lasval=1)
    mtext(expression(paste(italic("F") ~ (year^{-1}))),las=3,side=2,line=3,cex=1.0,lwd=1.75)
  ...
  # F at age, associated with fish capture and post-release mortality
  if (GrowthModelType ==  1 | GrowthModelType == 3) { # single sex
    plot(midpt, res$FAtLenDisc*DiscMort, "l", main="F (discard mortality)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
         ylim=c(0,ymax), col="red")
    AddAxesAndTickLabelsToPlot(xmin=0, xmax, xint, ymin=0, ymax, yint, cexval=NA, cexaxisval=1, lwdval=1.75, lineval=-0.2, lasval=1)
    mtext(expression(paste(italic("F") ~ (year^{-1}))),las=3,side=2,line=2.5,cex=1.0,lwd=1.75)
  ...
  if (GrowthModelType ==  2 | GrowthModelType == 4) { # separate sex
    plot(midpt, res$FAtLenDisc_Fem*DiscMort, "l", main="F (discard mortality)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
         ylim=c(0,ymax), col="red")
    AddAxesAndTickLabelsToPlot(xmin=0, xmax, xint, ymin=0, ymax, yint, cexval=NA, cexaxisval=1, lwdval=1.75, lineval=-0.2, lasval=1)
    mtext(expression(paste(italic("F") ~ (year^{-1}))),las=3,side=2,line=2.5,cex=1.0,lwd=1.75)
  ...
  if (GrowthModelType ==  1 | GrowthModelType == 3) { # single sex
    plot(midpt, res$FAtLenDisc, "l", main="F (capture + release)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
         ylim=c(0,ymax), col="red")
    AddAxesAndTickLabelsToPlot(xmin=0, xmax, xint, ymin=0, ymax, yint, cexval=NA, cexaxisval=1, lwdval=1.75, lineval=-0.2, lasval=1)
    mtext(expression(paste(italic("F") ~ (year^{-1}))),las=3,side=2,line=2.5,cex=1.0,lwd=1.75)
  ...
  if (GrowthModelType ==  2 | GrowthModelType == 4) { # separate sex
    plot(midpt, res$FAtLenDisc_Fem, "l", main="F (capture + release)", cex.main=1.0, pch=1, cex=0.6,
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2),ylab=NA, frame=F, xlim=c(0,xmax),
         ylim=c(0,ymax), col="red")
    AddAxesAndTickLabelsToPlot(xmin=0, xmax, xint, ymin=0, ymax, yint, cexval=NA, cexaxisval=1, lwdval=1.75, lineval=-0.2, lasval=1)
    mtext(expression(paste(italic("F") ~ (year^{-1}))),las=3,side=2,line=2.5,cex=1.0,lwd=1.75)
...
PlotAgeLengthCatchCurve_MargLength <- function(params, RefnceAges, MLL, GrowthCurveType, SelectivityType, ObsRetCatchFreqAtLen, ObsRetCatchFreqAtLengthAndAge,
  ...
  if (is.vector(ObsRetCatchFreqAtLen)) {
    ...
    AddAxesAndTickLabelsToPlot(xmin=NA, xmax, xint, ymin=NA, ymax, yint, cexval=NA, cexaxisval=NA, lwdval=NA, lineval=NA, lasval=NA)
    if (is.na(yaxis_lab)) {
      yaxis_lab2 = "Proportion - Males"
    } else {
      yaxis_lab2 = yaxis_lab
    }
...
#' Get vectors of observed ages and lengths for plotting
#'
#' Get vectors of observed ages and lengths for plotting of growth for age and length based catch curve
#'
#' @keywords internal
#'
#' @param nTimeSteps c(Linf,vbK) von Bertalanffy growth parameters
#' @param TimeStep observed frequencies in length classes
#' @param nLenCl minimum frequency to include
#' @param midpt lower bounds of length classes
#' @param ObsRetCatchFreqAtLen upper bounds of length classes
#'
#' @return ObsAge, ObsAge_F, ObsAge_M, ObsLenClRetCatchMidPt, ObsLenClRetCatchMidPt_F, ObsLenClRetCatchMidPt_M
#'
GetAgeLenVectors_AgeLengthCatchCurve_Growth <- function(nTimeSteps, TimeStep, nLenCl, midpt, ObsRetCatchFreqAtLen) {


  # generate data for individual fish (age classes and mid points of length classes)
  if (is.vector(ObsRetCatchFreqAtLen)) {
    SampleSize = sum(ObsRetCatchFreqAtLen)
    ObsAge = rep(NA, SampleSize); ObsAge_F = NA; ObsAge_M = NA
    ObsLenClRetCatchMidPt = rep(NA, SampleSize)
    ObsLenClRetCatchMidPt_F = NA; ObsLenClRetCatchMidPt_M = NA
  ...
  results = list(ObsAge=ObsAge,
                 ...
                 ObsAge_M=ObsAge_M,
                 ObsLenClRetCatchMidPt=ObsLenClRetCatchMidPt,
                 ObsLenClRetCatchMidPt_F=ObsLenClRetCatchMidPt_F,
                 ObsLenClRetCatchMidPt_M=ObsLenClRetCatchMidPt_M)
...
#'
#' Get lengths at age and associated quantiles from resampling, from multivariate normal distribution
...
#' @param ObsRetCatchFreqAtLen observed catch length frequency
#'
...
GetResampLengthsAtAge_AgeLengthCatchCurve_Growth <- function(nReps, nTimeSteps, PlotAges, GrowthCurveType, RefnceAges, params, vcov.params,
  ...
  if (!is.na(EstLenAtAge.sim[1,1])) {
    ...
    EstPropF.sim = NA; EstPropF.sim_low = NA; EstPropF.sim_up = NA
    EstPropM.sim = NA; EstPropM.sim_low = NA; EstPropM.sim_up = NA
  } else {
    EstProp.sim = NA; EstProp.sim_low = NA; EstProp.sim_up = NA
  ...
  results = list(EstProp.sim = EstProp.sim,
                 EstProp.sim_low = EstProp.sim_low,
                 EstProp.sim_up = EstProp.sim_up,
                 EstPropF.sim = EstPropF.sim,
                 EstPropF.sim_low = EstPropF.sim_low,
                 EstPropF.sim_up = EstPropF.sim_up,
                 EstPropM.sim = EstPropM.sim,
                 EstPropM.sim_low = EstPropM.sim_low,
                 EstPropM.sim_up = EstPropM.sim_up,
                 EstLenAtAge.sim = EstLenAtAge.sim,
                 EstLenAtAgeF.sim = EstLenAtAgeF.sim,
                 EstLenAtAgeM.sim = EstLenAtAgeM.sim)
...
#' @param FittedRes saved results from GetLengthBasedCatchCurveResults function (model will be refitted if set to NA)
#' @param nReps number of random parameter sets from parametric resampling to generate outputs with error
#' @param ShowLegend option to show or remove legend
#'
...
PlotAgeLengthCatchCurve_Growth <- function(params, RefnceAges, MLL, GrowthCurveType, SelectivityType, ObsRetCatchFreqAtLen, ObsRetCatchFreqAtLengthAndAge,
                                           lbnd, ubnd, midpt, SelectivityAtLen, DiscMort, MaxAge, NatMort, TimeStep, MainLabel,
                                           xaxis_lab, yaxis_lab, xmax, xint, ymax, yint, PlotCLs, FittedRes, nReps, ShowLegend) {
  ...
  # Get vectors of observed ages and lengths for plotting
  res = GetAgeLenVectors_AgeLengthCatchCurve_Growth(nTimeSteps, TimeStep, nLenCl, midpt, ObsRetCatchFreqAtLen)
  ObsAge = res$ObsAge; ObsAge_F = res$ObsAge_F; ObsAge_M = res$ObsAge_M
  ObsLenClRetCatchMidPt = res$ObsLenClRetCatchMidPt
  ObsLenClRetCatchMidPt_F = res$ObsLenClRetCatchMidPt_F; ObsLenClRetCatchMidPt_M = res$ObsLenClRetCatchMidPt_M
  ...
  EstPropF.sim = res$EstPropF.sim; EstPropF.sim_low = res$EstPropF.sim_low; EstPropF.sim_up = res$EstPropF.sim_up
  EstPropM.sim = res$EstPropM.sim; EstPropM.sim_low = res$EstPropM.sim_low; EstPropM.sim_up = res$EstPropM.sim_up
  ...
  if (is.vector(ObsRetCatchFreqAtLen)) { # combined sex
    ...
    if (is.na(yint)) yint = ylims$yint
    if (is.na(yaxis_lab)) {
      yaxis_lab2 = "Length (mm)"
    } else {
      yaxis_lab2 = yaxis_lab
    }
    plot(ObsAge_M, ObsLenClRetCatchMidPt_M, "p", main=MainLabel, cex.main=1.0, pch=16, cex=1.0, col=rgb(0,0,1,0.1),
         xaxt = "n", yaxt = "n", xlab=list(xaxis_lab,cex=1.2), ylab=list(yaxis_lab2,cex=1.2), frame=F, xlim=c(0,xmax), ylim=c(0,ymax))
...
PlotAgeLengthCatchCurve_Selectivity <- function(params, RefnceAges, MLL, GrowthCurveType, SelectivityType, ObsRetCatchFreqAtLen, ObsRetCatchFreqAtLengthAndAge,
  ...
  if (SelectivityType == 1) {
    SelectivityAtLen = res$ModelDiag$SelectivityAtLen
    EstProp.sim = SelectivityAtLen
    EstProp.sim_low = SelectivityAtLen
    EstProp.sim_up = SelectivityAtLen
  }
  if (SelectivityType > 1) {
    params = res$ModelDiag$params
    vcov.params = res$ModelDiag$vcov.Params
    ...
    colnames(SelAtLength.sim)=midpt
    SelAtLength2.sim = SelAtLength.sim

    for (j in 1:nReps) {
      ParamVals = exp(unlist(sims[j,]))

      if (SelectivityType == 2 & length(ParamVals)==10 | SelectivityType==3) {
        SelAtLength.sim[j,] = 1 / (1 + exp(-log(19) * (midpt - ParamVals[2]) /
                                             (ParamVals[4])))
        SelAtLength2.sim[j,] = 1 / (1 + exp(-log(19) * (midpt - ParamVals[3]) /
                                              (ParamVals[5])))
      }
      if (SelectivityType == 2 & length(ParamVals)!=10) {
        SelAtLength.sim[j,] = 1 / (1 + exp(-log(19) * (midpt - ParamVals[2]) /
                                             (ParamVals[3])))
      }
    ...
    if ((SelectivityType == 2 & length(ParamVals)==10) | SelectivityType == 3) {
        EstProp.sim2 = as.vector(apply(SelAtLength2.sim, 2, median))
        EstProp.sim2_low = as.vector(apply(SelAtLength2.sim, 2, quantile, probs = 0.025))
        EstProp.sim2_up = as.vector(apply(SelAtLength2.sim, 2, quantile, probs = 0.975))
    }
  } # selectivity type > 1
  ...
  if (PlotCLs == TRUE) {
    ...
    polygon(x,y, col="pink",border=NA)
    if ((SelectivityType == 2 & length(ParamVals)==10) | SelectivityType == 3) {
      sm1 = spline(midpt, EstProp.sim2_low, n=100, method="natural")
      sm2 = spline(midpt, EstProp.sim2_up, n=100, method="natural")
      sm1$y[which(sm1$y<0)]=0; sm1$y[which(sm1$y>1)]=1
      sm2$y[which(sm2$y<0)]=0; sm2$y[which(sm2$y>1)]=1
      x = c(sm1$x, rev(sm2$x)) # using shading for 95% CLs
      y = c(sm1$y, rev(sm2$y))
  ...
  if ((SelectivityType == 2 & length(ParamVals)==10) | SelectivityType == 3) {
    sm1 = spline(midpt, EstProp.sim2, n=100, method="natural")
    sm1$y[which(sm1$y<0)]=0; sm1$y[which(sm1$y>1)]=1
    lines(sm1$x,sm1$y, col="blue", cex=0.6)
    points(midpt, EstProp.sim2, col="blue", pch=1, cex=0.6)
  }
  AddAxesAndTickLabelsToPlot(xmin=NA, xmax, xint, ymin=NA, ymax, yint, cexval=NA, cexaxisval=NA, lwdval=NA, lineval=NA, lasval=NA)
  ...
  if ((SelectivityType == 2 & length(ParamVals)==10) | SelectivityType == 3) {
    ...
    L502est=paste("L50_2 =",round(exp(params[3]),0),"mm")
    L952est=paste("L95_2-L50_2 =",round(exp(params[5]),0),"mm")
    legend("topleft", pch=-1, legend=c(L50est, L95est, L502est, L952est), lty="solid",col="black",
...
#' Get parameter estimates and associated 95% CLs, in normal space, for age-based catch curve with
#' logistic selectivity
#'
#' This function returns estimated parameters, in normal space, for the age-based catch curve with
#' logistic selectivity (which are then outputted from the function GetLogisticCatchCurveResults).
#'
#' @keywords internal
#'
...
GetParamEst_LogisticCatchCurve <- function(params, ses) {
  ...
  if (length(params)==3) { # single sex - multinomial
    EstFMort = c(exp(params[1]), exp(params[1] + c(-1.96, 1.96) * ses[1]))
    EstSelA50 = c(exp(params[2]), exp(params[2] + c(-1.96, 1.96) * ses[2]))
    EstSelDelta = c(exp(params[3]), exp(params[3] + c(-1.96, 1.96) * ses[3]))
  ...
  if (length(params)==4) { # single sex - Dirichlet multinomial
    ...
    EstSelDelta = c(exp(params[3]), exp(params[3] + c(-1.96, 1.96) * ses[3]))
    EstSelA95 = EstSelA50[1] + EstSelDelta[1]
    temp = c(params[4], params[4] + c(-1.96, 1.96) * ses[4])
    EstTheta = 1/(1+exp(-temp));
  ...
  if (length(params)==5) { # two sexes - multinomial
    EstFMort = c(exp(params[1]), exp(params[1] + c(-1.96, 1.96) * ses[1]))
    EstSelA50_1 = c(exp(params[2]), exp(params[2] + c(-1.96, 1.96) * ses[2]))
    EstSelA50_2 = c(exp(params[3]), exp(params[3] + c(-1.96, 1.96) * ses[3]))
    EstSelDelta_1 = c(exp(params[4]), exp(params[4] + c(-1.96, 1.96) * ses[4]))
    EstSelDelta_2 = c(exp(params[5]), exp(params[5] + c(-1.96, 1.96) * ses[5]))
  ...
  if (length(params)==6) { # two sexes - Dirichlet multinomial
    ...
    EstSelA95_1 = EstSelA50_1 + EstSelDelta_1
    EstSelA95_2 = EstSelA50_2 + EstSelDelta_2
    temp = c(params[6], params[6] + c(-1.96, 1.96) * ses[6])
    EstTheta = 1/(1+exp(-temp));
    ParamEst = t(data.frame(FMort = round(EstFMort, 3), SelA50_1 = round(EstSelA50_1,3),
                            SelA50_2 = round(EstSelA50_2,3), EstSelDelta_1 = round(EstSelDelta_1, 3),
  ...
  result = list(ParamEst=ParamEst,
                EstFMort = EstFMort,
                EstSelA50 = EstSelA50,
                EstSelA95 = EstSelA95,
                EstSelA50_1 = EstSelA50_1,
...
#' Get selectivity at age, and fishing and total mortality at age
#'
#' This function returns values for selectivity at age, and fishing and total mortality at age
#' (which are then subsequently outputted in the function GetLogisticCatchCurveResults)
#'
#' @keywords internal
#'
#' @param params model parameters log(c(FMort, SelA50, SelDelta) for multinomial NLL or
...
#' @param EstSelA95 selectivity parameter (single sex)
#' @param EstSelA50_1 selectivity parameter (two sexes)
#' @param EstSelA95_1 selectivity parameter (two sexes)
#' @param EstSelA50_2 selectivity parameter (two sexes)
#' @param EstSelA95_2 selectivity parameter (two sexes)
#'
...
#' Get estimates of parameters with uncertainty and frequency at age from resampling
#'
#' This function returns values estimates of parameters with uncertainty and frequency at age from resampling
#' (which are then subsequently outputted in the function GetLogisticCatchCurveResults)
#'
#' @keywords internal
#'
...
#' Return Dirchlet multinomial negative log-likelihood for an age-based catch curve
#'
#' This function returns the Dirichlet multinomial negative log-likelihood, given observed age
#' frequency data and expected proportions at age, given parameters, for a catch curve with
...
GetLogisticCatchCurveResults <- function (params, NatMort, Ages, ObsAgeFreq)
{
  ...
  if (nSexes==1) SampleSize = sum(ObsAgeFreq)
...
PlotAgeBasedCatchCurveResults_LogSpace <- function(RecAssump, SpecRecAge, MinFreq, MinAge, MaxAge, NatMort,
  ...
  AddAxesAndTickLabelsToPlot(xmin=0, xmax, xint, ymin, ymax, yint, cexval=NA, cexaxisval=1, lwdval=0, lineval=0.2, lasval=1)
...
#' @keywords internal
#'
#' @param GrowthCurveType 1=von Bertalanffy, 2=Schnute, 3=annual increment input
#' @param MaxModelAge maximum age considered by model
#' @param Ages ages considered by model
#' @param GrowthParams growth parameters of either von Bertalanffy, Schnute model or inputs from tagging analysis
#' @param RefnceAges reference ages for Schnute model, set to NA if using von Bertalanffy model
#'
...
CalcMeanSizeAtAge <- function(GrowthCurveType, MaxModelAge, Ages, GrowthParams, RefnceAges) {
  ...
  if (GrowthCurveType == 3) {
    FemLenAtAge=NA; MalLenAtAge=NA
    MeanSizeAtAge[1,] = GrowthParams$MeanSizeAtAge[1,]
    MeanSizeAtAge[2,] = GrowthParams$MeanSizeAtAge[2,]
  }
...
GetLTMInputsForPerRecruitAnalysis <- function(GrowthCurveType, TimeStep, MaxAge, GrowthParams, RefnceAges, midpt) {
  ...
  if (GrowthCurveType == 3) { # annual size increment data inputted, from tagging model
    MeanEndingLength = GrowthParams$MeanEndingLength
    TimestepGrowthSizeInc = GrowthParams$TimestepGrowthSizeInc
  }
...
#'                              ret_L50, ret_L95, EstRetenAtLen, DiscMort, Steepness, SRrel_Type, NatMort, FMort)
#'
#' # Per recruit analysis using growth inputs from a tag-recapture analysis
#' library(WAFishBiology)
#' # Simulate tag-recapture data for growth analysis
#' set.seed(123)
#' nstep = 50 # number of steps for numerical integration
...
#' }
#' MeanEndingLength_TagRes = midpt + TimestepGrowthSizeInc_TagRes
#' # Per recruit analysis using growth inputs from tag recapture analysis
...
#' midpt = lbnd + (LenInc/2)
#' nLenCl = length(midpt)
#' GrowthCurveType = 3 # 1 = von Bertalanffy, 2 = Schnute, 3 = any model with annual length increment input
#' Linf <- NA # mm - von Bertalanffy growth model parameters - Females, males
#' vbK <- NA # year-1 - von Bertalanffy growth model parameters - Females, males
#' tzero <- NA # years - von Bertalanffy growth model parameters - Females, males
#' # input age information from tag recapture model
#' MeanSizeAtAge_Fem=MeanSizeAtAge_TagRes[seq(2,MaxAge+1,1)]; MeanSizeAtAge_Mal=MeanSizeAtAge_TagRes[seq(2,MaxAge+1,1)]
#' MeanSizeAtAge = t(data.frame(MeanSizeAtAge_Fem=MeanSizeAtAge_Fem,MeanSizeAtAge_Mal=MeanSizeAtAge_Mal))
#' colnames(MeanSizeAtAge) = 1:MaxAge
#' TimestepGrowthSizeInc_Fem=TimestepGrowthSizeInc_TagRes; TimestepGrowthSizeInc_Mal=TimestepGrowthSizeInc_TagRes
#' TimestepGrowthSizeInc = t(data.frame(TimestepGrowthSizeInc_Fem=TimestepGrowthSizeInc_Fem,TimestepGrowthSizeInc_Mal=TimestepGrowthSizeInc_Mal))
#' colnames(TimestepGrowthSizeInc) = midpt
#' MeanEndingLength_Fem=MeanEndingLength_TagRes; MeanEndingLength_Mal=MeanEndingLength_TagRes
#' MeanEndingLength = t(data.frame(MeanEndingLength_Fem=MeanEndingLength_Fem,MeanEndingLength_Mal=MeanEndingLength_Mal))
#' colnames(MeanEndingLength) = midpt
#' GrowthParams = list(MeanSizeAtAge=MeanSizeAtAge, TimestepGrowthSizeInc=TimestepGrowthSizeInc, MeanEndingLength=MeanEndingLength)
#' RefnceAges = NA
#' CVSizeAtAge = c(0.05, 0.05)
#' lenwt_a <- 0.00008 # combined sexes - weight (g) vs length (mm, TL) relationship parameters
#' ln_lenwt_a <- NA # for log-log relationship
#' lenwt_b <- 2.93 # combined sexes - weight (g) vs length (mm, TL) relationship parameters
#' WLrel_Type <- 1 # 1=power, 2=log-log relationship
#' EstWtAtLen <- data.frame(EstFemWtAtLen=NA,
#'                          EstMalWtAtLen=NA) # weight at age, inputted as values in data frame
#' ReprodScale <- 1.43 # 1=default (standard calculations for spawning biomass), 2=hyperallometric reproductive scaling with female mass (i.e. BOFFF effects)
#' ReprodPattern <- 1 # 1 = gonochoristic (separate sexes), 2 = protogynous (female to male sex change), 3 = protandrous (male to female sex change)
#' InitRatioFem <- 0.5 # Ratio of females to males at recruitment age/length
#' FinalSex_L50 <- NA # Logistic sex change relationship parameters (inflection point)
#' FinalSex_L95 <- NA # Logistic sex change relationship parameters (95% of max probability)
#' EstSexRatioAtLen <- NA  # sex ratio at length inputted as vector
#' EggFertParam <- NA # (NA or from ~0.2-1) NA = no effect, ~0.2 = direct effect of popn. sex ratio changes on egg fertilisation rates, 1 = no effects
#' mat_L50 <- c(120, 120) # females, males - Logistic length (mm) at maturity relationship parameters
#' mat_L95 <- c(130, 130) # females, males - Logistic length (mm) at maturity relationship parameters
#' EstMatAtLen <- data.frame(EstFemMatAtLen=NA,
#'                           EstMalMatAtLen=NA) # maturity at length, inputted as values in data frame
#' sel_L50 <- c(60, 60) # females, males - Logistic length selectivity relationship parameters
#' sel_L95 <- c(80, 80) # females, males - Logistic length selectivity relationship parameters
#' EstGearSelAtLen <- data.frame(EstFemGearSelAtLen=NA,
#'                               EstMalGearSelAtLen=NA)
#' ret_Pmax <- c(1.0, 1.0) # maximum retention, values lower than 1 imply discarding of fish above MLL
#' ret_L50 <- c(150, 150) # females, males - Logistic fish retention at length parameters
#' ret_L95 <- c(160, 160) # females, males - Logistic fish retention at length parameters
#' EstRetenAtLen <- data.frame(EstFemRetenAtLen=NA,
#'                             EstMalRetenAtLen=NA)
#' DiscMort <- 0 # discard mortality (e.g. 50% released fish die = 0.5)
#' Steepness <- 0.5 # steepness parameter of the Beverton and Holt stock-recruitment relationship
#' SRrel_Type <- 1 # 1 = Beverton-Holt, 2=Ricker
#' NatMort = 0.15 # Strain et al (2017)
#' FMort = NatMort
#' Res=CalcYPRAndSPRForFMort_LB(MaxModelAge, TimeStep, lbnd, ubnd, midpt, nLenCl, GrowthCurveType, GrowthParams,
#'                              RefnceAges, CVSizeAtAge, lenwt_a, ln_lenwt_a, lenwt_b, WLrel_Type, EstWtAtLen,
#'                              ReprodScale, ReprodPattern, InitRatioFem, FinalSex_L50, FinalSex_L95, EstSexRatioAtLen,
...
PlotPerRecruitResults_AB <- function(MaxModelAge, TimeStep, Linf, vbK, tzero, EstLenAtAge,
  ...
  # plot growth curve
  if (PlotOpt==0 | PlotOpt==1) {
    ylims = Get_yaxis_scale(c(c(0,Res$ModelDiag$FemLenAtAge),c(0,Res$ModelDiag$MalLenAtAge)))
  ...
  # plot weight at age
  if (PlotOpt==0 | PlotOpt==2) {
    ylims = Get_yaxis_scale(c(Res$ModelDiag$FemWtAtAge, Res$ModelDiag$MalWtAtAge))
    ymax = ylims$ymax; yint = ylims$yint
    xlims = Get_xaxis_scale(Res$ModelDiag$Ages)
    xmax = xlims$xmax; xint = xlims$xint
  ...
  if (PlotOpt==0 | PlotOpt==4) {
    ...
    legend('bottomright', col=c("blue","black"), legend=c("Mal. prop_mat","Mal. sel_land"),
           lty=c("solid","solid"),bty='n', cex=0.8,lwd=1.75)
    AddAxesAndTickLabelsToPlot(xmin=NA, xmax, xint, ymin=NA, ymax=1, yint=0.5, cexval=NA, cexaxisval=0.8, lwdval=1.5, lineval=NA, lasval=NA)
    mtext(expression(paste(plain("Proportion"))),las=3,side=2,line=2.5,cex=0.7,lwd=1.75)
    mtext(expression(paste(plain("Age (y"),plain(")"))),las=1,side=1,line=2,cex=0.7,lwd=1.75)
...
PlotPerRecruitResults_LB <- function(MaxModelAge, TimeStep, lbnd, ubnd, midpt, nLenCl, GrowthCurveType, GrowthParams,
  ...
  # plot growth curve
  if (PlotOpt==0 | PlotOpt==1) {
      temp = c(Growth_95PLs$FemLenAtAge_hi, Growth_95PLs$MalLenAtAge_hi)
      ylims = Get_yaxis_scale(temp[which(!is.na(temp))])
      ymax = ylims$ymax; yint = ylims$yint
      xlims = Get_xaxis_scale(Ages)
      xmax = xlims$xmax; xint = xlims$xint
  ...
  if (PlotOpt==0 | PlotOpt==2) {
    ylims = Get_yaxis_scale(c(c(0,Res$ModelDiag$FemWtAtLen),c(0,Res$ModelDiag$MalWtAtLen)))
    ymax = ylims$ymax; yint = ylims$yint
...
PlotPerRecruit_Biom_no_err_AB <- function(MaxModelAge, TimeStep, Linf, vbK, tzero, EstLenAtAge, lenwt_a, ln_lenwt_a,
  ...
  AddAxesAndTickLabelsToPlot(xmin=0, xmax, 0.5, ymin=0, ymax=1, yint=0.2, cexval=1, cexaxisval=1, lwdval=1.75, lineval=0.5, lasval=1)
