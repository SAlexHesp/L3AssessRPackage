...
# **************************
 ...
 #' Calculate length at age, from Schnute growth function
 #'
 #' @keywords internal
 #' @param Age Schnute parameters and reference ages (t1, t2, y1, y2, a, b)
 #' @return length at specified age (y)
 ...
 #'
 #' @keywords internal
 #' @param Fish length, Schnute parameters and reference ages (FishLen, t1, t2, y1, y2, a, b)
 #' @return Age at specifed length (Age)
 ...
 #'
 #' @keywords internal
 #' @param type, growth parameters, reference ages (for Schnute function), initial lengths
...
#' Res=GetLengthBasedCatchCurveResults(params, GrowthCurveType, GrowthParams, RefnceAges, SelectivityType, ObsCatchFreqAtLen,
#'                                           lbnd, ubnd, midpt, SelectivityVec, Linf, vbK, CVSizeAtAge, MaxAge, NatMort)
#' @export
GetLengthBasedCatchCurveResults <- function (params, GrowthCurveType, GrowthParams, RefnceAges, SelectivityType, ObsCatchFreqAtLen,
                                             lbnd, ubnd, midpt, SelectivityVec, Linf, vbK, CVSizeAtAge, MaxAge, NatMort)
{
  ...
  # inverse lgit transformed value
  ...
  res = AgeAndLengthBasedCatchCurvesCalcs(params, GrowthCurveType, GrowthParams, RefnceAges, CVSizeAtAge, CatchCurveType,
                                          SelectivityType, lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort)
...
#'
#' @keywords internal
#' @param params params (Vector), GrowthCurveType (Integer), GrowthParams (data frame), RefnceAges, CVSizeAtAge,
#' CatchCurveType (Integer), SelectivityType (Number), lbnd (Vector), ubnd (Vector), midpt (Vector), SelectivityType (Integer),
#' MaxAge (Number), NatMort (Number)
                                               ...
                                               SelectivityType, lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort)
{
  ...
  if (CatchCurveType == 2) { # age and length based catch curve
    ...
    # cat("AgeAndLengthBasedCatchCurvesCalcs: Linf",Linf,'\n')
  ...
  if (GrowthModelType == 1) { # von Bertalanffy, single sex
    MeanSizeAtAge = Linf * (1 - exp (-vbK * (1:MaxAge))) # as calculated from a von Bertalanffy growth curve
    MeanEndingLength = midpt + (Linf - midpt) * (1 - exp(-vbK)) # as calculated from a von Bertalanffy growth curve
    AnnGrowthSizeInc = MeanEndingLength-midpt # amount of annual growth with respect to initial length
  }
  if (GrowthModelType == 2) { # von Bertalanffy, separate sex
    ...
    for (i in 1:2) {
      MeanSizeAtAge[i,] = Linf[i] * (1 - exp (-vbK[i] * (1:MaxAge))) # as calculated from a von Bertalanffy growth curve
      MeanEndingLength[i,] = midpt + (Linf[i] - midpt) * (1 - exp(-vbK[i])) # as calculated from a von Bertalanffy growth curve
  ...
  if (GrowthModelType ==  1 | GrowthModelType == 3) { # single sex
    ...
    InitRecNumber = 1.0
    CatchCurveResults = CalcCatches_AgeAndLengthBasedCatchCurves_cpp(params, NatMort, RecLenDist, InitRecNumber, MaxAge, nLenCl, midpt, SelAtLength, LTM)

    # expected prop at length
    CatchAtLen = CatchCurveResults$CatchAtLen
    ExpCatchPropInLenClass = CatchAtLen / sum(CatchAtLen)
  ...
  if (GrowthModelType ==  2 | GrowthModelType == 4) { # separate sex
    ...
    CatchCurveResults_Mal = CalcCatches_AgeAndLengthBasedCatchCurves_cpp(params, NatMort, RecLenDist_Mal, InitRecNumber, MaxAge, nLenCl,
                                                                         midpt, SelAtLength, LTM_Mal)

    # expected prop at length
    CatchAtLen_Fem = CatchCurveResults_Fem$CatchAtLen
    CatchAtLen_Mal = CatchCurveResults_Mal$CatchAtLen
    CatchAtLen = CatchAtLen_Fem + CatchAtLen_Mal
    ExpCatchPropInLenClass_Fem = CatchAtLen_Fem / (sum(CatchAtLen_Fem) + sum(CatchAtLen_Mal)) # prop (with respect to all fish)
    ExpCatchPropInLenClass_Mal = CatchAtLen_Mal / (sum(CatchAtLen_Fem) + sum(CatchAtLen_Mal)) # prop (with respect to all fish)
    ExpCatchPropInLenClass = CatchAtLen / sum(CatchAtLen)

    # expected prop at age
    CatchAtLengthAndAge_Fem = CatchCurveResults_Fem$Catch
    ExpCatchAtAge_Fem = rowSums(CatchAtLengthAndAge_Fem)
    ExpCatchPropAtAge_Fem = ExpCatchAtAge_Fem / sum(ExpCatchAtAge_Fem)
    CatchAtLengthAndAge_Mal = CatchCurveResults_Mal$Catch
    ExpCatchAtAge_Mal = rowSums(CatchAtLengthAndAge_Mal)
    ExpCatchPropAtAge_Mal = ExpCatchAtAge_Mal / sum(ExpCatchAtAge_Mal)

    # expected prop at age, given length
    Catch_Fem = CatchCurveResults_Fem$Catch
    Catch_Mal = CatchCurveResults_Mal$Catch
    ...
    for (i in 1:MaxAge) {
      ExpCatchPropLengthGivenAge_Fem[i,] = Catch_Fem[i,] / sum(Catch_Fem[i,])
      ExpCatchPropLengthGivenAge_Mal[i,] = Catch_Mal[i,] / sum(Catch_Mal[i,])
    }
    ExpCatchAtAge=NA
...
#' Simulate some age frequency data with specified selectivity and mortality parameters.
#'
#' @param params Required sample size (SampleSize), Minimum and maximum ages (MinAge, MaxAge), asymptotic
#' logistic selectivity parameters (SelA50, SelA95), natural and fishing mortality mortality (NatMort, FMort)
...
#' (Number), L50 (Number), L95 (Number), SelectivityVec(Vector), MeanSizeAtAge (Vector), AnnGrowthSizeInc (Vector),
#' CVSizeAtAge (Number)
...
#'                          L50, L95, SelectivityVec, GrowthCurveType, GrowthParams, RefnceAges, CVSizeAtAge)
#' @export
SimLenAndAgeFreqData <- function(SampleSize, MaxAge, NatMort, FishMort, MaxLen, LenInc, SelectivityType,
                                 L50, L95, SelectivityVec, GrowthCurveType, GrowthParams, RefnceAges, CVSizeAtAge) {
  ...
  # relative catches at length
  CatchAtLen_Fem = colSums(CatchAtLengthAndAge_Fem)
  CatchAtLen_Mal = colSums(CatchAtLengthAndAge_Mal)
  CatchAtLen = CatchAtLen_Fem + CatchAtLen_Mal
  # catch proportions at length
  ExpCatchPropAtLen_Fem = CatchAtLen_Fem / sum(CatchAtLen_Fem)
  ExpCatchPropAtLen_Mal = CatchAtLen_Mal / sum(CatchAtLen_Mal)

  # generate observed catch frequencies at age
  SampleSize_Fem = SampleSize * (sum(CatchAtLen_Fem) / (sum(CatchAtLen_Fem) + sum(CatchAtLen_Mal)))
  ...
  SampleSize_Mal = SampleSize * (sum(CatchAtLen_Mal) / (sum(CatchAtLen_Fem) + sum(CatchAtLen_Mal)))
...
#' lbnd (Vector), ubnd (Vector), midpt (Vector), L50 (Number), L95 (Number), SelectivityVec (Vector),
...
#' InitL95 = 700
#' params = c(InitFishMort_logit, log(InitL50), log(InitL95))
#' FittedRes=GetLengthBasedCatchCurveResults(params, GrowthCurveType, GrowthParams, RefnceAges, SelectivityType, ObsCatchFreqAtLen,
#'                                           lbnd, ubnd, midpt, SelectivityVec, Linf, vbK, CVSizeAtAge, MaxAge, NatMort)
...
#'                                  L50, L95, SelectivityVec, GrowthCurveType, GrowthParams, RefnceAges, MaxAge, NatMort, MainLabel=NA,
#'                                  xaxis_lab=NA, yaxis_lab=NA, xmax=NA, xint=NA,
#'                                  ymax=NA, yint=NA, PlotCLs=TRUE, FittedRes)
#' @export
PlotLengthBasedCatchCurveResults <- function(params, SelectivityType, ObsCatchFreqAtLen, lbnd, ubnd, midpt,
                                             L50, L95, SelectivityVec, GrowthCurveType, GrowthParams, RefnceAges,
                                             MaxAge, NatMort, MainLabel, xaxis_lab, yaxis_lab, xmax, xint,
                                             ymax, yint, PlotCLs, FittedRes) {
...
#'
#' This function provides a plot of the fit of an age and length-based catch curve model
#' to marginal length composition data. The model has length-based selectivity, based on
#' either 1) specified gillnet selectivity parameters from the Kirkwood and Walker (1986) model
...
#' L50 (Number), L95 (Number), Linf (Number), vbK (Number), CVSizeAtAge (Number), MaxAge (Number),
#' NatMort (Number)
...
#'                                                 lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort)
#' # Plot. Note, can skip above step and set FittedRes=NA (plot function will be slower)
#'PlotAgeLengthCatchCurve_MargLength(params, SelectivityType, ObsCatchFreqAtLen, ObsCatchFreqAtLengthAndAge,
#'                                   lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort, MainLabel=NA,
#'                                   xaxis_lab=NA, yaxis_lab=NA, xmax=NA, xint=NA,
#'                                   ymax=0.10, yint=0.02, PlotCLs=TRUE, FittedRes)
#' @export
PlotAgeLengthCatchCurve_MargLength <- function(params, SelectivityType, ObsCatchFreqAtLen, ObsCatchFreqAtLengthAndAge,
                                               lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort, MainLabel,
  ...
  #          bty='n', cex=0.6,lwd=-1, y.intersp=1.0, adj=0)
  # }
  # if (SelectivityType==2) { # logistic selectivity
  #   L50est=paste("L50 =",round(exp(params[2]),0),"mm")
  #   L95est=paste("L95 =",round(exp(params[3]),0),"mm")
...
#'
#' This function provides a plot of the growth curve estimated from an age and length-based
#' catch curve model. The model has length-based selectivity, based on
#' either 1) specified gillnet selectivity parameters from the Kirkwood and Walker (1986) model
#' or 2) estimated selectivity parameters for an asymptotic logistic selectivity curve. The model is
#' fitted to a sample of fish length and age data, by minimising the overall negative log-likelihood,
#' including the NLL associated with the marginal length composition and a conditional age at length NLL,
...
#' @param params params (Vector), SelectivityType (Number), ObsCatchFreqAtLen (Vector),
#' ObsCatchFreqAtLengthAndAge (data frame), lbnd (Vector), ubnd (Vector), midpt (Vector),
#' L50 (Number), L95 (Number), Linf (Number), vbK (Number), CVSizeAtAge (Number), MaxAge (Number), NatMort (Number)
#' @return Plot of fitted length-based catch curve to age and length composition data
#' @examples
#' # Simulate data
#' set.seed(123)
...
#'                                                 lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort)
#' # plot
#' PlotAgeLengthCatchCurve_Growth(params, SelectivityType, ObsCatchFreqAtLen, ObsCatchFreqAtLengthAndAge,
#'                                lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort, MainLabel=NA,
#'                                xaxis_lab=NA, yaxis_lab=NA, xmax=40, xint=10,
#'                                ymax=1000, yint=200, PlotCLs=TRUE, FittedRes)
#' @export
PlotAgeLengthCatchCurve_Growth <- function(params, SelectivityType, ObsCatchFreqAtLen, ObsCatchFreqAtLengthAndAge,
                                           lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort, MainLabel,
                                           xaxis_lab, yaxis_lab, xmax, xint,
                                           ymax, yint, PlotCLs, FittedRes) {
  ...
  EstLenAtAge.sim = data.frame(matrix(nrow = 500, ncol = length(0:MaxAge)))
  ...
  #   Kest = bquote("k =" ~ .(Kval) ~ y^-1)
  #   CVest=paste("CV =",round(exp(params[4]),3))
  #   legend("bottomright", pch=-1, legend=c(as.expression(Fest),Linfest,
  #          as.expression(Kest), CVest), lty="solid",col="black",
  #          bty='n', cex=0.6,lwd=-1, y.intersp=1.0, adj=0)
  # }
  # if (SelectivityType==2) { # logistic selectivity
  ...
  #   Kest = bquote("k =" ~ .(Kval) ~ y^-1)
  #   CVest=paste("CV =",round(exp(params[6]),3))
  #   legend("bottomright", pch=-1, legend=c(as.expression(Fest), L50est, L95est, Linfest,
  #                                          as.expression(Kest), CVest), lty="solid",col="black",
  #          bty='n', cex=0.6,lwd=-1, y.intersp=1.0)
  # }
}
...
#' # plot
#' PlotAgeLengthCatchCurve_Selectivity(params, SelectivityType, ObsCatchFreqAtLen, ObsCatchFreqAtLengthAndAge,
#'                                     lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort, MainLabel=NA,
#'                                     xaxis_lab=NA, yaxis_lab=NA, xmax=NA, xint=NA,
#'                                     ymax=NA, yint=NA, PlotCLs=TRUE, FittedRes)
#' @export
PlotAgeLengthCatchCurve_Selectivity <- function(params, SelectivityType, ObsCatchFreqAtLen, ObsCatchFreqAtLengthAndAge,
                                                lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort, MainLabel,
                                                xaxis_lab, yaxis_lab, xmax, xint,
                                                ymax, yint, PlotCLs, FittedRes) {
  ...
  if (SelectivityType == 2) {
    ...
    set.seed(123)
    sims = data.frame(MASS::mvrnorm(n = 500, params, vcov.params))
    SelAtLength.sim = data.frame(matrix(nrow = 500, ncol = length(midpt)))
...
#' ObsCatchFreqAtLen2 (Vector), Age_midptlencl (Vector), ln_n_dt (Vector)
...
#' Length-converted catch curve with Schnute growth function and knife edge selectivity
#'
#' Return outputs associated with length converted catch curve method with Schnute growth curve
#' function and knife edge selectivity, required to fit the model
#' @keywords internal
#' @param Estimated parameters - fishing mortality (params),
#' Schnute growth parameters (GrowthParams), growth reference ages (RefnceAges),
#' observed catch length frequency (ObsCatchFreqAtLen), minimum frequency for catch curve (MinFreq),
#' lower bounds, midpoints and upper bounds of length classes (lbnd, midpt, ubnd)
#' @return length class corresponding to peak frequency (PeakLencl), last length class considered
#' in analysis (LastLenCl), expected proportions in each length class (ExpProp)
...
#' @param Estimated parameters (params), model type, 1=Pauly's method, 2=Schnute growth with
#' logistic selectivity, 3=Schnute growth with knife edge selectivity (ModelType), reference ages
#' for Schnute growth curve (RefnceAges),Schnute growth parameters (GrowthParams), observed catch
#' length frequency (ObsCatchFreqAtLen), minimum frequency for catch curve (MinFreq), lower bounds,
#' midpoints and upper bounds of length classes (lbnd, midpt, ubnd)
...
#' 95 percent confidence limits for Est_ln_n_dt
#' @examples
#' # Simulate data
#' SampleSize=1000
#' set.seed(123)
...
#' ZMort <- 0.3
#' yIntercept <- 10
#' params = c(log(ZMort),yIntercept)
#' ModelType = 2 # 1 = von Bertalanffy growth curve (Pauly), 2 = length-converted catch curve - Schnute growth curve)
#' res=GetLenConvCatchCurveResults(params, ModelType, GrowthParams, RefnceAges, ObsCatchFreqAtLen,
#'                                 MinFreq, lbnd, midpt, ubnd)
#' @export
GetLenConvCatchCurveResults <- function(params, ModelType, GrowthParams, RefnceAges, ObsCatchFreqAtLen,
                                        MinFreq, lbnd, midpt, ubnd) {
...
#' @param Estimated parameters (params), maximum age (MaxAge), model type, 1=Pauly's method, 2=Schnute growth with
#' logistic selectivity, 3=Schnute growth with knife edge selectivity (ModelType), reference ages
#' for Schnute growth curve (RefnceAges), Mean size at age from growth curve (MeanSizeAtAge), observed catch
#' length frequency (ObsCatchFreqAtLen), minimum frequency for catch curve (MinFreq), lower bounds,
#' midpoints and upper bounds of length classes (lbnd, midpt, ubnd)
...
#' res=GetLenConvCatchCurveResults(params, ModelType, GrowthParams, RefnceAges, ObsCatchFreqAtLen,
#'                                 MinFreq, lbnd, midpt, ubnd)
#' PlotLenConvCatchCurveResults(params, MaxAge, ModelType, GrowthParams, RefnceAges, ObsCatchFreqAtLen,
#'                             MinFreq, lbnd, midpt, ubnd)
#' @export
PlotLenConvCatchCurveResults <- function(params, MaxAge, ModelType, GrowthParams, RefnceAges, ObsCatchFreqAtLen,
                                           MinFreq, lbnd, midpt, ubnd) {
    ...
    plot(res$Age_midptlencl,res$Obs_ln_n_dt,"p",frame.plot=F, xlim=c(min(Ages),max(Ages)),
         ylim=c(0,ymax),col="black", ylab="ln(n/dt)",xlab="Relative age")
...
#' as estimated from an age and length-based  catch curve model. The model has length-based selectivity, based on
#' either 1) specified gillnet selectivity parameters from the Kirkwood and Walker (1986) model
#' or 2) estimated selectivity parameters for an asymptotic logistic selectivity curve. The model is
#' fitted to a sample of fish length and age data, by minimising the overall negative log-likelihood,
#' including the NLL associated with the marginal length composition and a conditional age at length NLL,
...
#' # Note, can skip above step and set FittedRes=NA (plot function will be slower)
#' # plot
#' PlotAgeLengthCatchCurve_Cond_AL(params, SelectivityType, ObsCatchFreqAtLen, ObsCatchFreqAtLengthAndAge,
#'                                 lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort, MainLabel=NA,
#'                                 xaxis_lab=NA, yaxis_lab=NA, xmax=40, xint=10,
#'                                 ymax=NA, yint=NA, FittedRes)
#' @export
PlotAgeLengthCatchCurve_Cond_AL <- function(params, SelectivityType, ObsCatchFreqAtLen, ObsCatchFreqAtLengthAndAge,
                                            lbnd, ubnd, midpt, SelectivityVec, MaxAge, NatMort, MainLabel,
                                            xaxis_lab, yaxis_lab, xmax=40, xint=10,
                                            ymax, yint, FittedRes) {
...
#' @param params RecAssump - must be 0 (peak age) or 1 (peak age + 1), MinAge, MaxAge (integers), ObsAgeFreq
#' for Ages=1:MaxAge (Numeric Vector)
