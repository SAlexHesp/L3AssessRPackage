...
#### Selectivity and mortality calculations

Selectivity is either estimated and by an asymptotic logistic curve, or imposed (inputted as a vector). It is assumed that length based selectivity is common to both sexes. Applying the model assuming logistic selectivity, $S_l$, the selectivity of fish of, of either sex, of length $L$, corresponding to the midpoint of length class $l$, is

$$
S_l = 1/(1+\exp(-\log_e(19)(L-L_{50}^{\mathrm{sel}})/(L_{95}^{\mathrm{sel}}-L_{50}^{\mathrm{sel}})))
$$

where $L_{50}^{\mathrm{sel}}$ and $L_{95}^{\mathrm{sel}}$ are the lengths at which 50 and 95% of fish of sex $s$ are selected by the gear. The fishing mortality for fish of either sex in length class, $l$, is

$$
F_l = S_lF^*
$$
where $F^*$ is the instantaneous rate of fishing mortality ($y^-1$), for fully-selected fish. The total mortality at length is thus
...
#### Survival and catch calculations
...
$$
\hat{C}_{s,l,a} = (F_{s,l}/Z_{s,l})(1-\exp(-Z_{s,l}))N_{s,l,a}
$$
...
F_{s,t}=\psi_{s,a}F
$$
where $F$ is the fully-selected fishing mortality. Conversely, for model scenarios allowing for post-release mortality (i.e. $D>0$), $F_{s,a}$ is calculated as

$$
F_{s,a} = (\phi_{s,a}+\delta_{s,a}D)F
$$
where $\phi_{s,a}$ and $\delta{s,a}$ are the selectivity of fish landings and fish discards, respectively, for fish of sex $s$ and age $a$.
...
$$
C_{s,a} = N_{s,a}\biggl(\frac{F_{s,a}}{Z_{s,a}}\biggr)(1-\exp(-Z_{s,a}))
$$
...
$$
where $P_{max}^{s=1}$, the terminal sex, is females. For hermaphroditic species, the values of $N_{s,a}$ become modified, prior calculation of survival at the next age step, to reflect the assumed stable sex ratio. The modified values for female, $N'_{s=1,a}$ are calculated as
...
$$
where $\theta_s$ and $\theta_{0,s}$ is the spawning biomass per recruit for sex $s$, for the fished and unfished population, respectively. Spawning potential ratio, for sexes combined, is thus calculated as
...
$$
where $\phi_{s,l}$ and $\delta_{s,l}$ are the selectivity of fish landings and fish discards, respectively, for fish of sex $s$ at the midpoint of length class $l$.
...
$$
where $n$ donotes the number of length classes considered by the model. The catch of fish of sex $s$ of at the midpoint of length class $l$, i.e. $C_{s,l}$, is calculated from the Baranov catch equation.
$$
C_{s,l} = N_{s,l}\biggl(\frac{F_{s,l}}{Z_{s,l}}\biggr)(1-\exp(-Z_{s,l}))
$$
...
As with age-based per recruit anlaysis, spawning potential ratio, $SPR$, for sex $s$, is calculated as
$$
SPR_s = \frac{\theta_s}{\theta_{0,s}}
$$
where $\theta_s$ and $\theta_{0,s}$ is the spawning biomass per recruit for sex $s$, for the fished and unfished population, respectively. Spawning potential ratio, for sexes combined, is calculated as
$$
SPR_{s=1:2} = \frac{\theta_{s=1}+\theta_{s=2}}{\theta_{0,{s=1}}+\theta_{0,{s=2}}}
$$
