...
# Introduction
#### Last updated February 2025 (Alex Hesp - DPIRD)
...
Several data simulation routines have been implemented for model testing. The include length and/or age generation from equilibrium population models, and more sophisticated dynamic modelling approaches, allowing for a specified history of exploitation and level of annual recruitment variation. 
...
$$
\hat{Z} = \ln\bigg[1+\overline{X}-\frac{1}{n}\bigg]-\ln\overline{X}-\bigg[\frac{{(n-1)(n-2)}}{n(t+1)(n+t-1)}\bigg]
$$
...
### Multi-year catch curve analysis allowing for variable recruitment

This catch curve model allows estimation of fishing mortality from age composition data whilst accounting for recruitment variation. A minimum of two years of age data (consecutive 'biological' years or not far apart) is required. This model, based on the 'relative abundance analysis' described by Deriso et al. (1985), has been used in multiple fish biological studies and assessments in Western Australia (e.g. Coulson et al., 2009; Fairclough et al., 2014; Smith et al., 2024). Mortality is assumed to be constant. The model is fitted to estimate fishing mortality and annual recruitment deviations for specified year classes. As, potentially, a substantial number of parameters is estimated, this model uses RTMB for optimisation, available with the R package RTMB (Kristensen, 2025). The current implementation assumes knife edge selectivity.

Applying this model, the expected number of fish $\hat{N}_{a,t}$ of year class $y$ of age $a$ years at the beginning of the first biological year (i.e. typically defined as mean birth date of one year to the next), $t1$, is calculated as

$$
\begin{align}
    \hat{N}_{a,t} = R_{t_1} && \text{if a=0}\\
    \hat{N}_{a,t} = R_{t_1}\exp\bigg[-\sum_{j=0}^{a-1}Z_{j,t_1-a+j} \bigg] && \text{if a<0<A}\\
    \hat{N}_{a,t} = \bar{R}\exp\bigg[-\sum_{j=0}^{A-1}Z_{j,t_1-A+j} \bigg] /(1-\exp[-Z_{A,t_1}]) && \text{if a=A}\\
\end{align}
$$
where $Z$ denotes total mortality, $j$ is an index of age (where $0 ≤ j ≤ A$), $R_{t_1}$ is the number of zero year old fish recruiting to the population in year $t_1$, $\bar{R}$ is the average recruitment level (i.e. 1, in normal space). $A$ is a plus group containing all fish of age $A$ years or older. In following biological years

$$
\begin{align}
    \hat{N}_{a,t} = R_{t} && \text{if a=0}\\
    \hat{N}_{a,t} = \hat{N}_{a-1,t-1}\exp[-Z_{a-1,t-1}] && \text{if a<0<A}\\
    \hat{N}_{a,t} = \hat{N}_{a-1,t-1}\exp[-Z_{a-1,t-1}] + \hat{N}_{A,t-1}\exp[-Z_{A,t-1}] && \text{if a=A}\\
\end{align}
$$
$R_y$, the number of age zero recruits in year class $y$, is calculated as

$$
\begin{align}
    \hat{R}_y = \bar{R}\exp\big[\epsilon_y-\frac{\sigma_R^2}{2} \big] && \text{for estimated y}\\
    \hat{R}_y = \bar{R} && {\text{for } y\leq{t_1}-A} \text{ and } y>t_1 \\
\end{align}
$$
Above, $\epsilon_y$ refers to the estimated ‘recruitment deviation’ for year class $y$. $\epsilon_y~N(o,\sigma^2_R)$ , where $\sigma^2_R$ is the variance of the natural logarithms of the recruitment deviations, estimated as model parameters. The expected catches at age $a$ in year $t$ are calculated from the Baranov catch equation, i.e. 

$$
\hat{C}_{a,t}=\frac{F_{a,t}}{Z_{a,t}}\big[1-\exp(-Z_{a,t})\big]\hat{N}_{a,t}
$$

$\hat{P}_{a,t}$, the expected proportion of fish of age $a$ in the catch in year $t$ is calculated as

$$
\hat{P}_{a,t}=\hat{C}_{a,t}/\sum_{j=0}^{A}\hat{C}_{a,t}
$$
The multinomial likelihood associated with the age composition data is calculated as

$$
\lambda_1=\sum_{t=t_1}^T\sum_{a=a_1}^Ap_{a,t}\ln{\hat{P}_{a,t}}
$$
where $\hat{P}_{a,t}$ is the observed proportion of fish in the catch at age $a$ in year $t$.

The log-likelihood associated with the annual recruitment deviations, is

$$
\lambda_2=\frac{1}{2\pi\sigma^2_R}\sum_y\epsilon^2_y
$$

The overall model likelihood, $\lambda$ is thus

$$
\lambda = \lambda_1 + \lambda_2
$$
The default value for $\epsilon^2_y$ is 0.6, as commonly assumed in many stock assessments. A penalty is also imposed to ensure the recruitment deviations sum to zero, to help prevent confounding between trends in recruitment deviations and fishing mortality. 

Key functions in L3Assess relevant to applying this method are:  
GetVarRecCatchCurveRes()  
Refer to help files and example code, showing function inputs and outputs.
...
\end{align}
$$
where $n_ij$ is the number of fish in length class $j$ caught in mesh size $i$. The relative selectivity of fish in the multimesh research gill net in length class $j$ is estimated as
...
When fitting the model, natural mortality ($M$) may be estimated using an informative lognormal prior, allowing uncertainty in $M$ to be propagated through the analysis. Because the length composition data contain little or no information about natural mortality, the estimated value of $M$ is expected to remain close to the user-specified prior distribution. The prior is specified using a mean natural mortality ($\bar{M}$) and a standard deviation on the log scale ($\sigma_M$). Thus, 
$$ \log(M) \sim \mathrm{Normal}(\mu_M,\sigma_M) $$ 

where 

$$ \mu_M = \log(\bar{M}) - \frac{\sigma_M^2}{2} $$ 

so that the expected value of $M$ is 
$$ E(M) = \bar{M} $$ 
The corresponding lognormal penalty function associated with $M$ is 
$$ \lambda_M = \log(\sigma_M\sqrt{2\pi}) + \frac{ \left( \log(M)-\mu_M \right)^2 } {2\sigma_M^2}. $$
When estimating $M$, the overall objective function $\lambda_{\mathrm{T}}$ is thus 
$$ \lambda_{\mathrm{T}} = \lambda + \lambda_M $$

LBCC may also be fitted simultaneously to size compositions for retained fish and for fish of sizes that would typically be caught and released during normal fishing operations (e.g. if such fish were measured by researchers during onboard monitoring). In such cases, an additional multinomial likelihood is calculated, associated with the observed and expected length distributions for released fish. The model is then fitted to the summed likelihoods for retained and released fish. LBCC can also be fitted assuming a Dirichlet-multinomial (rather than multinomial distribution). The Dirichlet multinomial distribution has been implemented in the same way as described for the age-based logistic catch curve in L3Assess (see above), i.e. using the 'linear effective sample size' parameterisation, as recommended in Thorson et al. (2017) 
...
As with the length-based catch curve (LBCC), the natural mortality may be estimated when fitting the length and age-based catch curve, assuming that $M$ conforms to a lognormal distribution. In this case, an informative lognormal penalty prior for $M$ is added to the objective function. For more details, see description given for the LBCC method.
...
Deriso R. B., Quinn T. J. & Neal, P. R. (1985) Catch-age analysis with auxiliary information. Canadian Journal of Fisheries and Aquatic Sciences 42, 815–824. doi:10.1139/f85-104
...
Fairclough, D. V., Molony, B. W., Crisafulli, B. M., Keay, I. S., Hesp, S. A., & Marriott, R. J. (2014) Status of demersal finfish stocks on the west coast of Australia. Fisheries Research Report Number 253, Department of Fisheries, Perth, WA, Australia.
...
Kristensen, K. (2025). RTMB: 'R' Bindings for 'TMB'. R package version 1.7. https://CRAN.R-project.org/package=RTMB.
...
Smith, K. A., Hesp, S. A.. & Coulson, P. G. 2024. Variations in life-history traits in exploited and unexploited populations of Perth herring (Nematalosa vlaminghi): evidence for density-dependent growth? Marine and Freshwater Research 75. https://doi.org/10.1071/MF23134.
