...
# Introduction
#### Last updated January 2024 (Alex Hesp - DPIRD)
...
This sex-structured length-based catch curve approach allows for common or divergent growth between the two sexes, growth variation (by applying sex-specific length-transition matrices). Selectivity can be assumed to be common, or separate, for both sexes. Fully-selected fishing mortality (and natural mortality) are assumed to be common to both sexes. Growth can be based on either a von Bertalanffy growth curve or Schnute growth curve. The approach involves calculating expected survival and catches for each sex, per recruit, given specified values for natural mortality, growth parameters (and associated variation), weight-at-length parameters, and model parameters for selectivity and fishing mortality. 

#### Growth calculations - length-based catch curve

Fish are assumed to recruit into the population at age $t=1$ y (to help avoid the possibility of negative lengths), with the size distribution assuming to conform to a normal distribution. The proportion of fish of sex $s$, in length class $k$ at age $t=1$ is calculated as
...
g_{s,k,j} = \left\{
    \begin{array}{ll}
        \int_{{\infty}^-}^{\mathrm{l_{s,k}^+}}f[\omega_s(T,l_s,j)]dl & \mbox{if }k=1 \\
        \int_{l_{s,k}^-}^{\mathrm{l_{s,k}^+}}f[\omega_s(T,l_s,j)]dl &\mbox{if }k=1<k<n \\
\int_{l_{s,k}^-}^{\mathrm{\infty^+}}f[\omega_s(T,l_s,j)]dl &\mbox{if }k=n \\
    \end{array}
\right.
$$
where $l_s$ is the length for a fish of sex $s$, $f$ is the specified (normal) distribution, $l_{s,k}^-$ and $l_{s,k}^+$ are the lower and upper limits, respectively, of length class $j$ for fish of sex $s$, and $\omega_s$ is a vector of parameters dependent on the time period $T$ and sex, and the midpoint of the size class $j$, $l_j$. In the model, growth is applied at the end of each time step, after mortality has been applied.


### Gear selectivity, retention and discard mortality - length-based catch curve

A 'retention function' approach is used to account for the possibility of 'discard mortality' (Fairclough et al., 2021; Shertzer et al., 2021). The vulnerability of fish of sex $s$ at the midpoint $l$ of length class $k$ to the fishing gear. $V_{s,a}$ is calculated as

$$
V_{s,l} = 1/(1+\exp(-\log_e(19)(l_s-L_{50,s}^{\mathrm{V}})/(L_{95,s}^{\mathrm{V}}-L_{50,s}^{\mathrm{V}})))
$$

where $L_{50,s}^{\mathrm{V}}$ and $L_{95,s}^{\mathrm{V}}$ are the lengths at which 50 and 95% of fish of sex $s$ are selected by the gear. Note that, for more robust optimisation, $$L_{95,s}^{\mathrm{V}}-L_{50,s}^{\mathrm{V}}$$ is inputted by the user as a single parameter value.

If an MLL does not apply, the probability of any fish being retained, if caught, may set (by the user) to one, regardless of its sex and length. In cases where a minimum legal length (MLL) applies, the vast majority of fishers comply with this regulation and high-grading of fish is likely to be negligible, the probability, of fish of sex $s$ at the midpoint $l$ of length class $k$, being retained in catches, is set to zero or one, depending on whether the midpoint of its length class is below or above the MLL, respectively. The selectivity of landings, for fish of sex class $s$ in length class $l$, is calculated as

$$
\phi_{s,l}=\psi_{s,l}V_{s,l}
$$
where $\psi_{s,l}$ denotes the probability of fish being retained, according to its sex and length. The selectivity of discarding is calculated as
...
$$
where $D$ is the specified proportion of fish that die following capture and release. To allow for a possible effect of an MLL on selectivity with negligible discard mortality, the value for $D$ can be set to 0.001. The fishing mortality of fish of sex $s$ at the midpoint $l$ of length class $k$ associated with retention, $F_{s,l}^R$ is calculated as
...
Total mortality for fish of sex $s$ at the midpoint $l$ of length class $k$ is calculated as 
...
As currently set up, the length-based catch curve model (and also age and length-based catch curve - see below) assumes age 1 recruits are in equal proportions (0.5) for each sex, as typically assumed for gonochoristic species. Note, if growth is not sexually dimorphic, these models can also be applied to hermaphroditic species (if assuming common growth, selectivity and mortality for both sexes). The per recruit numbers of either sex in length class $k$ that have survived to age $a$, i.e. $N_{s,k,a}$, is

$$
N_{s,k,a} = \left\{
    \begin{array}{ll}
        0.5\theta_{k,s} & \mbox{if }a=1 \\
        N_{s,k,a-1}\exp(-Z_{s,l}) &\mbox{if }a=1<a<A \\
        N_{s,k,a-1}\exp(-Z_{s,l})/(1-\exp(-Z_{s,l})) &\mbox{if }a=A \\
    \end{array}
\right.
$$

where $l$ is the midpoint for a corresponding length class, $k$. The estimated numbers of fish of sex $s$ in length class $k$ at age $a$ in retained catches, as calculated from the Baranov catch equation, is

$$
\hat{C}_{s,k}^R = \sum_k(F_{s,l}^R/Z_{s,l})(1-\exp(-Z_{s,l}))N_{s,k,a}
$$

The numbers of fish that are caught and discarded, which may either survive or die, are calculated as

$$
\hat{C}_{s,k}^{D*} = (F_{s,l}^{D*}/Z_{s,l})(1-\exp(-Z_{s,l}))N_{s,k,a}
$$
...
The estimated proportion of fish that are caught, and then released (which may survive or die), may be calculated as 

$$
\hat{\rho} = \Bigg(\frac{\sum_{s}\sum_{k}\hat{C}_{s,k}^{D*}}{\sum_{s}\sum_{k}(\hat{C}_{s,k}^R+\hat{C}_{s,k}^{D*})}\Bigg)
$$

The mean size of fish that are caught, and then released (which may survive or die), may be calculated as

$$
\hat{\kappa} = \Bigg(\frac{\sum_{s}\sum_{l}\hat{C}_{s,l}^{D*}l_s}{\sum_{s}\sum_{l}\hat{C}_{s,l}^{D*}}\Bigg)
$$


#### Objective function calculations - length-based catch curve

The observed length composition for fish that are caught and retained is assumed to conform to either a multinomial distribution or Dirichlet-multinomial distribution. The expected proportions of fish in each length class among such fish, $\hat{P}_l^R$, for combined sexes, is

$$
\hat{P}_k^R = \sum_s\Bigg(\frac{\hat{C}_{s,k}^R}{\sum_{k}\hat{C}_{s,k}^R}\Bigg)
$$

The multinomial negative log-likelihood associated with the observed length composition data for retained fish, $\lambda_1$, is

$$
\lambda_1 = \sum_lC_l^R\log_e(\hat{P}_l^R)
$$
where $C_l^R$ is the observed retained catch frequency in length class $l$. 

If data for fish which are caught and released are included, the expected proportions of fish are calculated as described above for retained fish, replacing $\hat{C}_{s,a}^R$ with $\hat{C}_{s,a}^D*$. Likewise, the negative log likelihood, $\lambda_2$, associated with fish that are caught and released is calculated in the same way as for retained fish, replacing $C_l^R$ with $C^{D*}_l$ and $\hat{P}_l^R$ with $\hat{P}^{D*}$.
...
The overall negative log-likelihood, $\lambda$, is calculated as

$$
\lambda = \lambda_1 + \lambda_2
$$
with $\lambda_2$ set to zero if the associated model options are not selected by the user.

The Dirichlet-multinomial has been implemented in the same way for the length-based catch curve, i.e. using the 'linear effective sample size' parameterisation as recommended in Thorson et al. (2017). The associated model equations are as described for the age-based logistic catch curve (see above).
...
Note that this catch curve allows use of alternative model time step values (e.g. 1/12 of a year vs 1 year). The equations outlined are based on an an annual timestep). For shorter-lived species (e.g. < 10 y), it may be more appropriate to use a shorter model timestep (if samples are collected at intervals throughout the year), such that the modelled length compositions more closely match observed data, by allowing fish to grow throughout the year. 
...
$$
SPR_s = \frac{\Phi_s}{\Phi_{0,s}}
$$
where $\Phi_s$ and $\Phi_{0,s}$ is the spawning biomass per recruit for sex $s$, for the fished and unfished population, respectively. For gonochoristic species, $SPR$ is calculated based on females only. For hermaphroditic species, Spawning potential ratio, $SPR$ is calculated for combined sexes, i.e.

$$
SPR_{s=1:2} = \frac{\Phi_{s=1}+\Phi_{s=2}}{\Phi_{0,{s=1}}+\Phi_{0,{s=2}}}
$$

$\Phi_s$ is calculated as
$$
\Phi_s = \sum_{a=0}^{A}N_{a,s}W_{a,s}P_{a,s}
$$
...
Variation in growth is modelling using the same approach as described above for the length-based and length and age-based catch curve. For completeness, the equations are repeated here for length-based per recruit analysis.

Fish are assumed to recruit into the population at age $t=1$ years (to help avoid the possibility of negative lengths), with the size distribution assuming to conform to a normal distribution. For each sex, the proportion of recruits (among fish of that sex) in length class $k$ at age $t=1$ is calculated as
...
L_{s,a=1}{\sim}N(\mu,\sigma{^2)}
$$
$\sigma$ is calculated given a specified value for the coefficient of variation, $CV$, assumed to be constant with respect to mean fish lengths at age (common to both sexes). The associated value of the normal probability density function, is 
...
g_{s,k,j} = \left\{
    \begin{array}{ll}
        \int_{{\infty}^-}^{\mathrm{l_{s,k}^+}}f[\omega_s(T,l_s,j)]dl & \mbox{if }k=1 \\
        \int_{l_{s,k}^-}^{\mathrm{l_{s,k}^+}}f[\omega_s(T,l_s,j)]dl &\mbox{if }k=1<k<K \\
\int_{l_{s,k}^-}^{\mathrm{\infty^+}}f[\omega_s(T,l_s,j)]dl &\mbox{if }k=K \\
    \end{array}
\right.
$$
where $l_s$ is the length for a fish of sex $s$, $f$ is the specified (normal) distribution, $l_{s,k}^-$ and $l_{s,k}^+$ are the lower and upper limits, respectively, of length class $j$ for fish of sex $s$, and $\omega_s$ is a vector of parameters dependent on the time period $T$ and sex, and the midpoint of the size class $j$, $l_j$, and where $K$ is the number of length classes. In the model, growth is applied at the end of each time step, after mortality has been applied.

$W_l$, the weight of fish of sex $s$ at length midpoint $l$, in length class $k$, can be inputted as a vector of mean weights at the midpoints of each length class considered in the model, or calculated from inputted parameter values according to a weight-length relationship, either
...
where $a_s$ and $b_s$ are the sex-specific weight-length parameters, for a model fitted to weights measured in g and fish lengths measured in mm. The probability of a fish of sex $s$ being mature at the midpoint $l$ of length class $k$ is calculated as
...
As with the age-based per recruit model, a 'retention function' approach is used to account for such 'discard mortality' (Fairclough et al., 2021; Shertzer et al., 2021). The vulnerability of fish of sex $s$ at the midpoint $l$ of length class $k$ to the fishing gear, $V_{s,a}$, is calculated as

$$
V_{s,l} = 1/(1+\exp(-\log_e(19)(l_s-L_{50,s}^{\mathrm{V}})/(L_{95,s}^{\mathrm{V}}-L_{50,s}^{\mathrm{V}})))
$$

where $L_{50,s}^{\mathrm{V}}$ and $L_{95,s}^{\mathrm{V}}$ are the lengths at which 50 and 95% of fish of sex $s$ are selected by the gear. The probability of a fish (during normal fishing operations) of either sex at the midpoint $l$ of length class $k$ being retained (i.e. retention curve), $ψ_{s,l}$, is calculated as
...
The selectivity of fish landings, $\phi_{s,l}$ for sex $s$ at the midpoint $l$ of length class $k$, is calculated as
...
For model scenarios not allowing for post-release mortality of undersize fish, the proportion of fish that die following capture and release, $D$, is set to zero. In such cases, the annual fishing mortality associated with capture and retention of fish of sex $s$ at the midpoint $l$ of length class $k$ is calculated as
...
F_{s,l} = (\phi_{s,l}+\delta_{s,l}D)F
$$
where $\phi_{s,l}$ and $\delta_{s,l}$ are the selectivity of fish landings and fish discards, respectively, for fish of sex $s$ at the midpoint $l$ of length class $k$. The fishing mortality associated with retention, $F_{s,l}^R$, is calculated as
...
### Yield per recruit - length-based per recruit analysis

Yield per recruit, $YPR$, is calculated as
$$
YPR = \sum_{s=1}^{2}\sum_{k=1}^{K}C_{s,k}W_{s,l}
$$
where $K$ donotes the number of length classes considered by the model, and W_{s,l} is the expected weight for the midpoint $l$ of length class $k$. In the above equation, $C_{s,k}$ is the retained catch of fish of sex $s$ for fish in length class $k$, calculated from the Baranov catch equation, i.e.
$$
C_{s,k} = N_{s,k}\biggl(\frac{F_{s,l}^R}{Z_{s,l}}\biggr)(1-\exp(-Z_{s,l}))
$$
where $F_{s,l}$ is the length-dependent fishing mortality associated with retention for sex $s$, and $Z_{s,l}$ is the length dependent total mortality for sex $s$, calculated for the midpoint of length class $k$.

For a model with an annual timestep, $N_s,a,k$, i.e. the numbers of fish of sex $s$ at age $a$ at midpoint $l$ in length class $k$, for the fished population are calculated as
$$
\begin{align}
    N_{s,a,l} = \xi && \text{if s=1 and a=1}\\
    N_{s,a,l} = 1-\xi && \text{if s=2 and a=1}\\
    N_{s,a,l} = N_{s,a-1,l}\exp(-Z_{s,a-1,l}) && \text{if 1<a<A}\\
    N_{s,a,l} = N_{s,a-1,l}\exp(-Z_{s,a-1,l})/(1-\exp(-Z_{s,a-1,l})) && \text{if a=A}\\
\end{align}
$$
where $\xi$ is the ratio of females to males at age 1 yr, and $A$, is the maximum age considered by the model. Note that, for this length-based per recruit analysis, a juvenile is recruited at age one rather than at age zero (to avoid calculations of negative values of fish lengths). The numbers of sex $s$ at the midpoint $l$ of length class $k$ for the unfished population, $\acute{N}_{s,k}$ are calculated as above for $N_{s,k}$, but replacing all values for total mortality with natural mortality, $M$. 

If the species is a protogynous hermaphrodite (female to male sex change), the proportion of females at the midpoint $l$ of length class $k$ is assumed to remain constant, and follow the following logistic relationship 
...
$$
where $P_{max}^{s=1}$, the terminal sex, is females. For hermaphroditic species, the values of $N_{s,a,l}$ become modified, prior calculation of survival at the next age step, to reflect the assumed stable sex ratio. The modified values for female, $N'_{s=1,l}$ are calculated as
$$
...
### Spawning potential ratio - length-based per recruit analysis

As with age-based per recruit analysis, spawning potential ratio, $SPR$, for sex $s$, is calculated as
$$
SPR_s = \frac{\Phi_s}{\Phi_{0,s}}
$$
where $\theta_s$ and $\theta_{0,s}$ is the spawning biomass per recruit for sex $s$, for the fished and unfished population, respectively. In the above equation, $\theta_s$ is calculated as
$$
\Phi_s = \sum_{k=1}^{K}N_{k,s}W_{l,s}P_{l,s}
$$
where $N_{k,s}$ is the number of fish of sex $s$ in length class $k$, and $W_{l,s}$ and $P_{l,s}$, respectively, are the weight and proportion of mature fish of sex $s$ at the midpoint $l$ of length class $k$.


### Calculating equilibrium recruitment - age and length-based per recruit models
...
R = \frac{S}{Sb+a}
$$
where $R$ is recruitment, $S$ is (in most cases, see below) female spawning biomass and $a$ and $b$ are parameters of the stock-recruitment relationship. This equation may be reparameterised into an alternative form that employs a steepness parameter $h$, i.e., the proportion of the unexploited equilibrium recruitment expected to recruit when spawning biomass is reduced to 20% of its unfished equilibrium level (Mace and Doonan, 1988). For gonochoristic species, the equilibrium level of recruitment for the fished population, relative to that for the unfished population, $R_{eq}$, may be calculated as

$$
R_{eq} = \frac{4h\Phi_{s=1}-(1-h)\Phi_{0,s=1}}{5(h-0.2)\Phi_{s=1}}
$$
...
$$
B_{rel,s} =S_{eq,s}/\Phi_{0,s}
$$


### Calculation of biological reference points - age and length-based per recruit models

The expected value of relative biomass $B_{rel}$ at $B_{MSY}$ is calculated as the level at which $C_{eq}$ is maximised. The (equilibrium) level of fishing mortality expected to result in stock biomass being at $B_{MSY}$, i.e. $F_{MSY}$, is determined as the level of fully-selected fishing mortality resulting in the maximum equilibrium catch level being achieved. Applying reference points commonly adopted by DPIRD, WA, the threshold level for biomass corresponds to the level of $B{rel}$ at $B_{MSY}$, with the limit set to half this level, and the target at 1.2 times this level (or higher). Reference points for fishing mortality (i.e. $F_{targ}$, $F_{thresh}$ and $F_{lim}$) correspond to the values of $F$ resulting in relative biomass being at the target, threshold and limit level, respectively. Alternatively, for biomass, proxy levels may be specified for plotting, with the limit set to $B_{rel}=0.2$, the threshold at $B_{rel}=0.3$ and target at $B_{rel}=0.4$.

### Allowing for error in per recruit calculations - age and length-based per recruit models

This implementation of per recruit analysis allows for incorporation uncertainty in estimates of fishing mortality, natural mortality and steepness, assuming independent normal or lognormal distributions for each of these parameters. Following the recommendation of Hamel & Cope (2022), for natural mortality, the example code specifies a lognormal distribtuion, with standard deviation value, in log space, of 0.31. For fishing mortality, the standard deviation value can be derived from outputs of catch curve analysis. Estimates of key outputs from per recruit analysis, with uncertainty, are produced using parametric sampling to produce a specified number (e.g. 500) of F, M and h values, which are inputted into the deterministic per recruit equations (i.e. within the CalcYPRAndSPRForFMort() function), to produce 500 sets of estimated results of key parameters of interest. Lower and Upper 95% confidence limits are taken as the 2.5 and 97.5 percentiles of the 500 estimates. 

Note, it is also possible to incorporate other sources of uncertainty, such as for growth and maturity curves, if estimates are available for the both parameters of each curve and associated variance-covariance matrix (such as are outputted by the WAFishBiolgy R package. 'Wrapper' code, involving use of the CalcYPRAndSPRForFMort() function for each set of input parameters, and storing and analysing the outputs to produce confidence limits, has not yet been implemented.     

### Risk calculations and associated table for relative biomass estimates - age and length-based per recruit models
...
The package provides a basic procedure in the SimAgeFreqData() function for simulating age frequency samples, based on the equations provided above describing the age-based catch curve method with logistic selectivity. Given a specified sample size, values for expected catch proportions at age, $\hat{P_a}$, an age sample can be generated from a multinomial distribution, i.e. using the rmultinom() function in R. This method for simulating age frequency data is provided in code for several examples for applying age-based catch curves. Note that these data generated for a stock assumed to at equilibrium with respect to mortality and recruitment, assuming logistic age-based selectivity.  

## Simulation of length and age data (assuming constant mortality and recruitment)

The function SimLenAndAgeFreqData() is provided for simulating length and age data, based on the equations and assumptions of the 'length and age based catch curve model'. The function produces length frequency samples of a specified sample size, given the expected proportion of fish in each length class $l$, i.e. $\hat{P}_l$, using the rmultinom() function in R. An associated age frequency sample for each length class $l$ is generated from the expected, conditional age distributions for each length class, $\hat{P}_{a|l}$, by applying the rmultinom() function from within R. Note that these data generated for a stock assumed to at equilibrium with respect to mortality and recruitment, assuming logistic length-based selectivity (and allowing for discarding, if specified).  

## Simulation of length data using a dynamic length-structured model

The function SimLenAndAgeFreqData_DynMod() is available for simulating length data, allowing for constant or variable mortality, and/or constant or variable recruitment, using a dynamic length-structured model. This length-based uses outputs from the extended, length-based per recruit analyses to calculate the stock recruitment relationship and specify the initial population at fished equilibrium.

## Description of dynamic length-structured model

Broadly, the dynamics of the population may be represented by the equation below, describing the numbers of
fish in the exploited population surviving to year $t$, i.e. $N_t$. That is,

$$
N_{t+1,s}=G_sS_{t,s}N_{t,s}+R_{t+1,s}
$$
where $G_s$ is a sex-specific length transition matrix, $S_{t,s}$ is the survival matrix for sex $s$ in year $t$ and $R_{t+1,s}$ is a vector for the numbers of fish of sex $s$ recruiting to the different size classes in year $t$.

Growth for sex $s$, is modelled using a sex-specific length transition matrix, i.e. $G_s=(g_{k,i,s})$, representing the probability of fish in length class $k$ growing into length class $i$, within a model timestep.
...
g_{2,1,s} & g_{2,2,s} & . &   &   \\ 
. & . & . & . & . \\ 
g_{K-1,1,s} & g_{K-1,2,s} & . & g_{K-1,K-1,s} &   \\ 
g_{K,1,s} & g_{K,2,s} & . & S_{K,K-1,s} & S_{K,K,s} \\ 
\end{bmatrix} 
$$

The survival matrix $S_s$, describes the proportions of fish of sex $s$ in each length class $k$ that
survive the combined effects of fishing and natural mortality in year $t$, i.e.

$$
S_{t,s}=\begin{bmatrix} 
S_{1,t,s} & 0 & 0 \\ 
0 & S_{2,t,s} & 0 \\ 
. & . & . \\ 
0 & 0 & S_{K,t,s} \\ 
...
$N_{t,s}$ is a vector, containing the number of fish $N_{t,k,s}$, of sex $s$ within length class $k$ of the $K$
different length classes comprising the length composition in year $t$, i.e.

$$
N_{t,s}=\begin{bmatrix} 
N_{1,t,s} \\ 
N_{2,t,s} \\
. \\
N_{K,t,s} \\
...
$R_{t,s}$ is also a vector of recruitment, for fish of sex $s$, resulting from spawning in year $t$, distributed over $K$ length classes, i.e.

$$
R_{t+1,s}=\begin{bmatrix} 
R_{1,t+1,s} \\ 
R_{2,t+1,s} \\
. \\
R_{K,t+1,s} \\
...
### Specifying an exploitation history (with stochasticity and autocorrelation)

The function SimExploitationHistory_DynMod() may be used to specify an exploitation history,
as input to the length-based dynamic model function SimLenAndAgeFreqData_DynMod(), to allow for variable mortality. For this routine, the user specifies an initial value for fishing mortality, then values for fishing mortality at the end of one or several periods, e.g. there the user may specify an initial period of increasing mortality, followed by a period of decreasing mortality and then a period of stable mortality. The values of fishing mortality between the initial and final year of each fishing period are calculated using linear interpolation. Stochasticity and autocorrelation in the fishing mortality series are introduced according to an AR1 process, i.e.

$$
F_t = A_FF_{t-1}+{\epsilon}_{t_F}
$$
where $A_F$ represents autocorrelation between successive F values and $\epsilon_{t_F}$ are  random, normally distributed errors. The values of $\epsilon_{t_F}$ are generated using the rnorm() function in R, specifying a mean of zero and standard deviation ${\sigma\epsilon}_{t_F}$, calculated as  

$$
{\sigma\epsilon}_{t_F} = \bigg(\sqrt{(1+A_F^2)}\bigg){\sigma_F}
$$
where the value for $\sigma_F$ is a value for standard deviation for fishing mortality input by the user. Thus, $\sigma_\epsilon$ increases as the level of autocorrelation increases. Note that fishing mortality may also be input to SimLenAndAgeFreqData_DynMod() as remaining constant, with or without error (as in Crisafulli et al., submitted).

### Generating recruitment deviations (with stochasticity and autocorrelation)

The function GetRecruitmentDeviations_DynSimMod() may be used to specify an recruitment deviations $\epsilon_{t_R}$ with random, auto-correlated errors, as input to the dynamic length-based model to allow recruitment variation. This routine requires the number of years for the simulation period, and specified levels for recruitment variation and auto-correlation. Stochasticity and autocorrelation are introduced according to an AR1 process, i.e.

$$
\epsilon_{t_R} = A_R\epsilon_{t-1}+{\epsilon'}_{t_R}
$$
where $A_R$ represents autocorrelation between successive recruitment deviation values, in log space, and ${\epsilon'}_{t_R}$ are  random, normally distributed errors. The values of ${\epsilon'}_{t_R}$ are generated using the rnorm() function in R, specifying a mean of zero and standard deviation ${\sigma{\epsilon}_{t_R}}$, calculated as  

$$
{\sigma{\epsilon'}_{t_F}} = \bigg(\sqrt{(1+A_R^2)}\bigg){\sigma_R}
$$
where the value for $\sigma_R$ is a value for standard deviation for recruitment deviations, in log space, input by the user. Thus, $\sigma_\epsilon$ increases as the level of autocorrelation increases.

### Calculation of annual recruitment

The parameters of the assumed Beverton-Holt stock-recruitment for the dynamic simulation model are calculated as

$$
\alpha = (B_0/R_0)\frac{1-h}{4h}
$$
and 
...
\beta=(h-0.2)/(0.8hR_0)
$$
In the above equations, $R_0$, is an arbitrary value for initial recruitment for the unfished population (e.g. 1000) and $B_0$ is the unfished biomass, calculated as

$$
B_0=\Phi_{0,s=1}R_0
$$
The value of female spawning biomass per recruit, $\theta_{s=1}$, is calculated using the length-based per recruit model.


$$
\begin{align}
    R_t = (\Phi_{s=1}-\beta)/(\beta-\Phi_{s=1}) && \text{if t=1}\\
    R_{t+1} = (B_t/(\alpha+{\beta}B_t))\epsilon_{t_R}-0.5\sigma_{R}^2\\
\end{align}
$$

### Calculating annual survival

The numbers of females of length $l$ at the midpoint of length class $k$, at the beginning of timestep $t+1$, after addition of juvenile recruits, is

$$
N_{{t+1},s=1,l}=N_{{t+1},s=1,l}+(\xi{R_{t+1,l}}\theta_{s=1,l})
$$
and the corresponding number of males is

$$
N_{{t+1},s=2,l}=N_{t+1,s=2,l}+((1-\xi){R_{t+1,l}}\theta_{s=2,l})
$$
### Calculating annual catches (in numbers)

The annual catch in year $t$, for sex $s$ in length class $k$, in numbers, is calculated as

$$
C_{t,s,k} = N_{t,s,k}\biggl(\frac{F_{t,s,l}^R}{Z_{t,s,l}}\biggr)(1-\exp(-Z_{t,s,l}))
$$

where $F_{t,s,l}$ is the length-dependent fishing mortality associated with retention in year $t$ for sex $s$, and $Z_{t, s,l}$ is the length dependent total mortality in year $t$ for sex $s$, both calculated for the midpoint $l$ of length class $k$. 

### Generating (simulated) annual length compositions
...
SimAgeFreqData()  
SimLenAndAgeFreqData()  
PlotSimLenAndAgeFreqData()  
SimExploitationHistory_DynMod()  
SimLenAndAgeFreqData_DynMod()  
