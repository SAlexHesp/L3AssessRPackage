...
$$
The (multinomial) negative log-likelihood associated with the length composition data, $\lambda_1$, is

$$
\lambda_1 = -\Big(C_l\log_e(\hat{P}_l)\Big)
$$
...
Two sources of auxillary information, that may be available for a fished stock, may be inputted, that might enable the parameters of a logisitic curve, for estimating $V_{s,l}$, to be estimated. These include estimates, from observed data, on the proportion of fish that are caught and released (and associated standard deviation), and/or on the mean length of fish (and associated standard deviation), of such fish.

The estimated proportion of fish that are caught, and then released (which may survive or die), is calculated as 

$$
\hat{\rho} = \Bigg(\frac{\sum_{s}\sum_{l}\hat{C}_{s,l}^{D*}}{\sum_{s}\sum_{l}(\hat{C}_{s,l}^R+\hat{C}_{s,l}^{D*})}\Bigg)
$$

The mean size of fish that are caught, and then released (which may survive or die), is calculated as

$$
\hat{\kappa} = \Bigg(\frac{\sum_{s}\sum_{l}\hat{C}_{s,l}^{D*}l_s}{\sum_{s}\sum_{l}\hat{C}_{s,l}^{D*}}\Bigg)
$$
The negative log-likelihood associated with a model estimate for $\hat{\rho}$, assumed to be normally distributed, is

$$
\lambda_2 = -\log_e\Bigg(\frac{1}{\sigma_\rho\sqrt{2{\pi}}}\exp\bigg[\frac{(\hat{\rho}-\rho)^2}{2\sigma_\rho^2}\bigg]\Bigg)
$$
where $\rho$ and $\sigma_{\rho}$ are the observed proportion and associated standard deviation inputted by the user, if available. 

Similarly, if an estimate for mean size of caught and released fish, and an associated standard deviation value are available, the negative-log likelihood for $\hat{\kappa}$ is

$$
\lambda_3 = -\log_e\Bigg(\frac{1}{\sigma_\kappa\sqrt{2{\pi}}}\exp\bigg[\frac{(\hat{\kappa}-\kappa)^2}{2\sigma_\kappa^2}\bigg]\Bigg)
$$
Note that, in reality, $\hat{\kappa}$ is likely to be right-skewed rather than normally-distributed, and thus the impact of using this option on results should be considered carefully.
...
$$
where $\lambda_2$ and $\lambda_3$ are set to zero if assocaited observed data are not inputted by the user.
