...
# Introduction
#### Last updated November 2023 (Alex Hesp - DPIRD)
...
The observed length composition for fish that are caught and retained is assumed to conform to a multinomial distribution. The expected proportions of fish in each length class, $\hat{P}_l^R$, for combined sexes, is

$$
\hat{P}_l^R = \sum_s\sum_a\Bigg(\frac{\hat{C}_{s,a}^R}{\sum_{l}\hat{C}_{s,a}^R}\Bigg)
$$
Similarly, the expected proportion of fish which are caught and released (which may subsequently survive or die), is

$$
\hat{P}^{D*}_l = \sum_s\sum_a\Bigg(\frac{\hat{C}^{D*}_{s,a}}{\sum_{l}\hat{C}^{D*}_{s,a}}\Bigg)
$$
...
$$
\lambda_1 = -\Big(C_l^R\log_e(\hat{P}_l^R)\Big)
$$
where $C_l^R$ is the observed retained catch frequency in length class $l$. Likewise, the negative log likelihood associated with fish that are caught and released is
$$
\lambda_2 = -\Big(C^{D*}_l\log_e(\hat{P}^{D*}_l)\Big)
$$
If an observed length composition is not available for fish that are released, but an estimate exists for the proportion of fish that are released in catches ($\rho$), fitting the model to these auxillary data may assist in estimating the logistic parameters for the vulnerability curve (i.e. gear selectivity). The negative log-likelihood associated with a model estimate for $\hat{\rho}$, assumed to be normally distributed, is

$$
\lambda_3 = -\log_e\Bigg(\frac{1}{\sigma_\rho\sqrt{2{\pi}}}\exp\bigg[\frac{(\hat{\rho}-\rho)^2}{2\sigma_\rho^2}\bigg]\Bigg)
$$
where $\rho$ and $\sigma_{\rho}$ are the observed proportion and associated standard deviation inputted by the user, if available. Whether this option remains is currently under review.
...
$$
with $\lambda_2$ and $\lambda_3$ set to zero if the associated model options are not selected by the user.
...
### Risk calculations and associated plots for relative biomass estimates from per recruit analysis


### Calculation of prediction intervals for growth curves used in length-based per recruit analysis


### Calculation of expected size sizes and prediction intervals at different levels of fishing
