...
#' @param CatchCurveType 1=length based, 2=age and length based
#' @param GrowthCurveType 1=von Bertalanffy, 2=Schnute
#' @param GrowthParams (von Bertalanffy or Schnute growth parameters), GrowthModelType
#' 1=von Bertalanffy 1 sex, 2= von Bertalanffy 2 sexes, 3=Schnute 1 sex, 4=Schnute 2 sexes, and
#' CVSizeAtAge for 1 or 2 sex model
#'
...
GetGrowthModelType <- function (CatchCurveType, GrowthCurveType, GrowthParams)
{
  ...
  Results = list(GrowthModelType=GrowthModelType)
                                               ...
                                               MLL, SelectivityType, lbnd, ubnd, midpt, SelectivityVec, DiscMort, MaxAge, NatMort, TimeStep)
{
  ...
  # get GrowthModelType
  if (CatchCurveType==1) { # length based
    res2 = GetGrowthModelType(CatchCurveType, GrowthCurveType, GrowthParams)
...
SimLenAndAgeFreqData <- function(SampleSize, MaxAge, TimeStep, NatMort, FishMort, MaxLen, LenInc, MLL, SelectivityType,
  ...
  # get inputs for length transition matrices
  Res = GetGrowthInputsForLengthTransitionMatrices(MaxAge, TimeStep, nLenCl, midpt, GrowthCurveType, GrowthModelType, GrowthParams, RefnceAges, SelectivityType)
  GrowthModelType = Res$GrowthModelType
  MeanSizeAtAge = Res$MeanSizeAtAge
  TimestepGrowthSizeInc = Res$TimestepGrowthSizeInc
  MeanEndingLength = Res$MeanEndingLength
  ...
  if (GrowthModelType==1 | GrowthModelType==3) { # single sex
    TimestepGrowthSizeInc_Fem = TimestepGrowthSizeInc
    TimestepGrowthSizeInc_Mal = TimestepGrowthSizeInc
    CVSizeAtAge_Fem = CVSizeAtAge
    CVSizeAtAge_Mal = CVSizeAtAge
    RecLenDist_Fem = RecLenDist
    RecLenDist_Mal = RecLenDist
  } else { # separate sex
    TimestepGrowthSizeInc_Fem = as.vector(unlist(TimestepGrowthSizeInc[1,]))
    TimestepGrowthSizeInc_Mal = as.vector(unlist(TimestepGrowthSizeInc[2,]))
    CVSizeAtAge_Fem = CVSizeAtAge[1]
  ...
  # get landed and discarded catches, and selectivity of landings and discards (retention function approach)
  InitRecNumber = 0.5
  ...
  # discarded catch at decimal age and length
  ...
  # total catch at decimal age and length (discards plus retained catches)
  ...
  # Discarded catch proportions at decimal age
  ...
  ObsAgeClRetCatch_Fem = rep(NA, SampleSize_Fem); ObsAgeClRetCatch_Mal = rep(NA, SampleSize_Mal); ObsAgeClRetCatch = rep(NA, SampleSize[1])
  ...
  ObsAgeClRetCatch_Fem = floor(round(ObsDecAgeRetCatch_Fem,6)); ObsAgeClRetCatch_Mal = floor(round(ObsDecAgeRetCatch_Mal,6))
  ObsAgeClRetCatch = c(ObsAgeClRetCatch_Fem, ObsAgeClRetCatch_Mal) # decimal ages combined sexes
  ...
  if (is.vector(GrowthParams)) { # params inputted as combined sex
    ObsRetCatchFreqAtLen_Fem = NA; ObsRetCatchFreqAtLen_Mal = NA
    RetCatchAtDecAge_Fem = NA; RetCatchAtDecAge_Mal = NA
    DiscCatchAtDecAge_Fem = NA; DiscCatchAtDecAge_Mal = NA
    ExpRetCatchPropAtDecAge_Fem = NA; ExpRetCatchPropAtDecAge_Mal = NA
    ExpRetCatchPropLengthGivenDecAge_Fem = NA; ExpRetCatchPropLengthGivenDecAge_Mal = NA
    ExpRetCatchPropDecAgeGivenLength_Fem = NA; ExpRetCatchPropDecAgeGivenLength_Mal = NA
    ObsRetCatchFreqAtLengthAndDecAge_Fem = NA; ObsRetCatchFreqAtLengthAndDecAge_Mal = NA
    ObsLenClRetCatchMidPt_Fem = NA; ObsLenClRetCatchMidPt_Mal = NA
  }
  ...
  Results = list(DecAges = DecAges,
                 ...
                 ObsRetCatchFreqAtLengthAndDecAge_Fem = ObsRetCatchFreqAtLengthAndDecAge_Fem,
                 ...
                 ObsRandLenRetCatch_Fem = ObsRandLenRetCatch_Fem,
...
PlotLengthBasedCatchCurve_Selectivity <- function(params, MLL, SelectivityType, ObsRetCatchFreqAtLen, lbnd, ubnd, midpt, SelectivityVec,
  ...
  plot(res$ModelDiag$mid, res$ModelDiag$SelAtLength, "p", main=MainLabel, cex.main=1.2, pch=1, cex=0.6,
  ...
  points(res$ModelDiag$mid, res$ModelDiag$SelAtLength, col="red", cex=0.6)
  axis(1, at = seq(0, xmax, xint), line = 0.2, labels = F)
  axis(2, at = seq(0, ymax, yint), line = 0.2, labels = F)
  axis(1, at = seq(0, xmax, xint), lwd = 0, labels = T, line = 0, cex.axis = 1, las = 1)
  axis(2, at = seq(0, ymax, yint), lwd = 0, labels = T, line = 0, cex.axis = 1, las = 1)
